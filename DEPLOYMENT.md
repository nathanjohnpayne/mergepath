...
**Recovery lane (#931).** The same two contexts also go unreported when the `pull_request` event that produces them is delayed or dropped by GitHub — measured at 6–32 minutes on `gaycruisebingo#662`, and as a complete drop on two heads there. `pr-review-policy-recovery.yml` therefore runs a `*/15` sweep that re-derives both contexts from live PR state and publishes them on the head of every open PR that has no check run for them. It is a separate workflow from `pr-review-policy.yml` on purpose: adding the schedule there would require gating that workflow's jobs off on non-`pull_request` events, and a skipped Actions job still materializes a check run under its own name — two of those names are required contexts, and a skipped conclusion satisfies a required check. A green `Label Gate` additionally re-derives Phase 4 applicability first, because the job that applies `needs-external-review` runs in the very workflow run the lane is recovering from, so an absent label is not evidence that no review was required. To recover a stuck PR immediately rather than waiting for the next interval, fire the dispatch:
