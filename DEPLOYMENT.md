...
3. **Preflight-injected shared ADC** — when no project SA key is provisioned or detectable, `scripts/op-preflight.sh --mode deploy` (or `--mode all`) materializes the shared 1Password ADC into a tempfile and exports its path as `OP_PREFLIGHT_ADC_TMPFILE`. Subject to the same RAPT-expiry surface as the shared ADC. If neither this nor step 2 yields a usable credential, `--mode deploy` fails closed, while `--mode all` still exports the PATs and records the failure in its session cache; later `--mode all` calls in the same Firebase-project context reuse that result for `OP_PREFLIGHT_DEPLOY_DEGRADED_BACKOFF_SECONDS` (default 900s) instead of re-prompting biometric, and `--refresh` retries immediately.
