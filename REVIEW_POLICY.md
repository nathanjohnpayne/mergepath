...
12a. `codex-review-request.sh` polls the PR until one of the following:
     ...
     > **Canonical completion check (#684/#1157).** Codex now has three observed output shapes across two endpoints, and they are NOT interchangeable: (1) a **findings round** — one or more unaddressed inline comments — is delivered as a `COMMENTED` review object on `pulls/{pr}/reviews`; (2) the legacy **clean/affirmative verdict** is an issue comment with `Codex Review:` and `Reviewed commit: <sha>` on `issues/{pr}/comments`; (3) the newer persistent **Codex Review Summary** is a marker-tagged issue comment (`<!-- codex-pull-request-review-summary -->`) that Codex edits in place from `Running` to `Completed`, with the abbreviated head in the Code Review row's Commit cell. Seeing nothing on one endpoint therefore does NOT mean Codex has not reviewed — it may have posted to the other one. Always read the UNION of both endpoints, filtered to `chatgpt-codex-connector[bot]`, and require exact-head evidence: review objects use `commit_id`, legacy verdicts use their `Reviewed commit:` prefix, and the summary uses its Code Review Commit cell. The summary's `updated_at` is its observation time because the bot mutates one comment in place. A `Running` summary and a 👀 reaction are liveness/ack evidence only; neither is completion or clearance. A `Completed` summary proves terminal delivery on that head to `codex-review-check.sh --diagnostic-signal-only`, but is not affirmative merge clearance because the row states completion, not disposition. Use the shipped helpers rather than hand-rolling a poll; a reviews-only watcher missed a legacy verdict on #567, an issue-comments-only watcher missed a findings review on #679, and a summary-blind diagnostic waited its entire budget after the clean swipewatch#105 pass in #1157.
