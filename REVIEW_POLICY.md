...
**`Label Gate` and `Self-Review Required` have a second producer: the recovery lane.** Those two are reported by `pull_request` only, so a delivery GitHub delays or drops leaves them at *"Expected — Waiting for status to be reported"* with no self-heal and no re-run path (#931). `.github/workflows/pr-review-policy-recovery.yml` re-derives both from live PR state every 15 minutes and on a `pr-review-policy-recheck` `repository_dispatch`, and publishes them through the Checks API. **It is a stand-in, never an owner**: once a native run exists on a head, the lane retires its own lineage with one non-blocking `neutral` run and stops writing. Two consequences if you are changing either gate — a recovered `Label Gate` will not go green without re-deriving Phase 4 applicability, because the job that applies `needs-external-review` runs in the same workflow run as the gate; and renaming either job breaks the lane's context lockstep, which `scripts/ci/check_pr_review_policy_recovery` fails on. The full contract, including the ownership table and the sweep's API budget, is [`specs/pr_review_policy_recovery.md`](specs/pr_review_policy_recovery.md).
