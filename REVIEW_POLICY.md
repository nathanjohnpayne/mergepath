...
> **Pushing a fix commit does NOT re-trigger a Codex review.** The Codex GitHub App reviews on PR-open, ready-for-review, and an explicit `@codex review` comment — a plain push (`synchronize`) is not on its documented trigger list. Step 14a's re-run of `codex-review-request.sh` is therefore mandatory on every round, not a formality: skip it and the prior round's stale HEAD remains the last review Codex ever produced, the merge gate's HEAD-anchored verdict check (step 16a) never sees a current clearance, and the PR sits red on `Merge clearance gate` with no review in flight and nothing arriving to fix it (#631).
