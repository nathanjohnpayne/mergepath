...
16a. Before merging, the agent runs `scripts/codex-review-check.sh <PR#>` to verify the merge gate. All of the following must be true:
     ...
     - **Gate (c)** — when `codex.enabled: true`, Codex has signaled clearance on the current HEAD via one of the three forms in step 12a (COMMENTED review with no unaddressed required-tier findings, a fresh 👍 reaction, or a HEAD-anchored affirmative issue-comment verdict with no unaddressed required-tier findings; #600), OR Codex previously posted an affirmative issue-comment verdict for a commit with the same external-review fingerprint as the current head and the current head has no newer Codex signal of its own (#705), OR a Phase 4b substitute clearance has landed (an `APPROVED` review on the current HEAD from a non-author identity in `available_reviewers`). The same-fingerprint carry-forward is limited to base-only/update-branch churn, and that limit is enforced by **two independent predicates** (#763) — one on the content, one on the range. **Content:** once external review is required, the fingerprint hashes the tree object IDs for every changed file in both the head and that head's merge base against the PR base, so any changed reviewed content or changed reviewed diff invalidates the carry-forward and requires fresh clearance. **Range:** every path whose content differs between the reviewed commit and the current head must additionally be base-derived — that is, it must not still be part of the PR's own net diff against its merge base. A path that changed since the reviewed commit *and* is still one of the PR's changed files carries content that verdict never saw, so carry-forward is refused and a current-head review is required. The range predicate fails closed: an unreadable or truncated (≥300-file) comparison refuses the carry-forward rather than assuming the range is clean. The fingerprint alone constrains the endpoints; the range predicate constrains the path between them. The Phase 4b substitute is the merge gate's understanding of Phase 4b clearance — without it, a PR that clears via Phase 4b would leave gate (c) failing forever and the auto-clear workflow would not remove `needs-external-review`. When `codex.enabled: false`, `scripts/codex-review-check.sh` ignores Codex bot reviews/reactions and gate (c) can clear only through the Phase 4b substitute path. Toggle via `codex.allow_phase_4b_substitute: true|false` in `.github/review-policy.yml` (default `true`; #218).
