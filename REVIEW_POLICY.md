...
1. **Wait for CodeRabbit.** CodeRabbit automatically posts a review when the PR is opened or updated. Prefer `scripts/coderabbit-wait.sh <PR#>` over an ad-hoc poll loop — the script anchors its "cleared" signal on the current HEAD, so it will not treat a stale review from a prior HEAD as current. Freshness resolves in three rungs: an exact SHA match wins outright — a review whose commit is the current `HEAD_SHA` counts regardless of its timestamp, including when the HEAD committer date is in the future (#824) — and the HEAD-committer-date floor applies only to a review carrying no matching SHA. Treating the floor as the sole test would discard a legitimate current-HEAD review and wait needlessly. Beneath those sits the third rung (#968): a summary whose own commits range names a commit that is not the current HEAD is a verdict about that other commit and never clears this head, however recently CodeRabbit edited it — the bot rewrites that comment in place for reasons unrelated to reviewing the new head, and an edit bumps `updated_at` without a re-review, which the floor alone reads as currency. A body naming no SHA at all still falls to the floor unchanged. The rungs are ORDERED, and the third yields to the first (#1003/#1022): a CodeRabbit review **object** pinned to the current HEAD by `commit_id` and carrying a clean report body satisfies the exact-SHA rung outright, and the mutable summary's commits range does not demote that head. Evaluating the demotion unconditionally inverted the published order and stalled a demonstrably-reviewed head to the advisory `exit 4`. The evidence is the #900 review-RUN selector rather than a bare `commit_id` match, so the body-LESS review object GitHub wraps around a single inline reply (CodeRabbit's `🐇 ✅` acknowledgement of a `[mergepath-resolve:…]` tag reply) never satisfies it (#919); neither does an unreadable reviews list, nor a run the finding counter never graded (#1031) — the counter scopes its inline findings to the newest head-pinned review object of ANY kind, so a body-less acknowledgement landing on top of a findings run makes it count zero under the acknowledgement while the rung would credit the run beneath it, and the rung is therefore satisfied only when the run it credits IS the object the counter graded. The demotion reads only the marker-selected summary's OWN commits range (#1023) — a range quoted by any other comment is not CodeRabbit's head claim and never vetoes a current-head summary. The script also handles CodeRabbit's rate-limit state, which the platform does NOT auto-retry (see nathanjohnpayne/mergepath#138), and CodeRabbit's auto-pause state (see nathanjohnpayne/mergepath#490). On exit code `0` CodeRabbit has cleared with no high-severity markers; on `2` it has findings to address; on `4` the `coderabbit.max_wait_seconds` grace window elapsed (the agent may log a warning and proceed since CodeRabbit is advisory); on `5` the rate-limit retry budget was exhausted — alert the human rather than proceed, *unless* the Codex failover engaged (`codex_failover_requested: true`; see the rate-limit → Codex failover paragraph below), in which case `5` is a non-blocking note and the PR proceeds via Phase 4a; on `6` CodeRabbit auto-review is skipped and not (re-)invocable — see the auto-pause / skip paragraph below. If exit `4` occurs and `coderabbit.status_probe_enabled` is true, the helper has posted `@coderabbitai, how is the review going?`, waited the bounded `coderabbit.status_probe_wait_seconds` window, and surfaced CodeRabbit's narrative reply in the output JSON's `status_probe` field. That reply is narration only; it is never counted as a review or clearance signal.
   ...
   **Rate-limit → Codex failover (exit `5`, #489).** When CodeRabbit reports rate-limited and `coderabbit.codex_failover_on_rate_limit` is true (the default), the helper requests `@codex review` once — HEAD-pinned and idempotent — by invoking `scripts/codex-review-request.sh --trigger-only` with `MERGEPATH_PHASE_4A_GATED=true`, so the PR advances via Codex (the real blocking gate) instead of idling on CodeRabbit's hourly allowance. It composes with `codex.request_by_default` (#486) but fires regardless of it, and it is time-boxed/self-reverting: a single trigger per run with no permanent Codex pin, so once CodeRabbit recovers the steady-state posture returns. The failover is recorded in the output JSON as `codex_failover_requested: true`. Its effect on the exit-`5` contract: a `rate_limit_stalled` that engaged the failover is a **non-blocking note**, not a hard human-alert — Codex was requested and the normal Phase 4a flow + `Merge clearance gate` carry the merge. Only an exit `5` with `codex_failover_requested: false` (the knob off, or `codex.enabled: false` so there is nothing to fail over to) still requires alerting the human. In the **agent-driven wait**, the agent (in-session) then proceeds via the normal Phase 4a flow. In the **auto-merge workflow** (`agent-review.yml`'s `Wait for CodeRabbit` step), which bridges the author token to the failover (`OP_PREFLIGHT_AUTHOR_PAT` ← `AUTHOR_MERGE_TOKEN`, so the author-attributed `@codex review` can post in CI), the exit-`5` downgrade is **scoped to externally protected PR heads** — the unit-tested, fail-closed `scripts/coderabbit-automerge-rate-limit-gate.sh` proceeds only when `codex_failover_requested: true` **and** `scripts/merge-clearance-gate.sh --derive-rate-limit-protection` returns `true`. That query uses the live head's intrinsic threshold/protected paths, label force-on, and propagation-lane exemption, never label events alone; it returns `true` when the `Merge clearance gate` external arm will really hold the merge — which since #772 requires that gate to be **enforced**, i.e. enabled in the governing policy AND observably a required status check on the PR's base branch — or when intrinsic external review applies and `codex-review-check.sh` already confirms current-head Codex/Phase-4b clearance (#713, the label-cleared-by-approval timing case). Config-enabled alone is not evidence the gate can hold anything: a repo can run with `codex.external_review_gate.enabled: true` while `Merge clearance gate` is absent from base-branch protection, in which case the check goes red and the PR merges anyway. Enforcement that cannot be **determined** (an unreadable protection surface, a token without the scope) counts as not-enforced and falls through to the clearance probe, which is positive proof — so an undeterminable read costs availability, never safety. **Unprotected or vacuously gated** rate-limit stalls (under threshold, no protected paths, no label, a lane-exempt verified head, or a gate that is enabled but not enforced) do not get the downgrade on the strength of the stall alone: merge clearance passes vacuously for them, so proceeding on that basis would let such a PR auto-merge with neither bot having reviewed it (#512 round-3 Codex finding). Since #825 they get a second, evidential question instead of an immediate block — the gate waits up to `CODERABBIT_RATE_LIMIT_GATE_CLEARANCE_WAIT_SECONDS` (default 840s, matching `codex.review_timeout_seconds`; `0` probes once) for the review the failover just requested to actually clear the head, verified by `scripts/codex-review-check.sh` with `CODEX_REVIEW_CHECK_SKIP_CI=1`, `CODEX_REVIEW_CHECK_REQUIRE_APPROVAL_ON_HEAD=1` and `CODEX_REVIEW_CHECK_ALLOW_PHASE_4B_SUBSTITUTE=false`. That last flag is what keeps the bar honest: without it gate (c) would accept the same reviewer `APPROVED` that armed auto-merge as a Phase-4b substitute, and an unreviewed PR would "prove" its own clearance. Only a real current-head Codex signal (👍, affirmative verdict, or clean review) proceeds; no clearance inside the budget, an infrastructure error, a missing PR/repo, or a missing checker all still block, and the final pre-merge re-verify re-derives the same predicate on the live head. This is the deadlock fix for the class where the composition *CodeRabbit rate-limited + under-threshold PR* blocked deterministically — waiting longer for CodeRabbit could not resolve it, because observed Fair Usage windows (2109s, 2398s) exceed the whole `coderabbit.max_wait_seconds` ceiling. It is deliberately NOT the #826 topology change (Codex primary, CodeRabbit deliberate). Opt out per repo with `coderabbit.codex_failover_on_rate_limit: false`.
