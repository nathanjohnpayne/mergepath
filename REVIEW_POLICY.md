...
Protective retraction is not merge authorization. The trusted approval-independence job may use its write-scoped `github.token` only with `gh pr merge --disable-auto` after a live read proves that an auto-merge request exists; it immediately verifies the same native author and exact head with an absent request. It retracts every durable arm, including a native non-shared one, because an eventual GitHub auto-merge cannot be bound to the reviewed base SHA. That bounded path cannot arm, enqueue, or merge a PR, does not depend on the optional `AUTHOR_MERGE_TOKEN`, and is self-contained so the first workflow-plus-helper propagation PR cannot call a new helper option from an older trusted default branch.
