...
The approval guard's retraction is intentionally self-contained in the trusted workflow and uses only the workflow's write-scoped token, so it works when `AUTHOR_MERGE_TOKEN` is absent and during the first consumer PR before the new continuation helper exists on that consumer's default branch. It reads auto-merge state before requiring policy identity, leaves native non-shared arms untouched, and can issue only `--disable-auto` followed by readback. Rollout still requires an audit of every open shared-author PR for both an `autoMergeRequest` and a `mergeQueueEntry`: GitHub CLI cannot dequeue an already queued PR through `--disable-auto`, so any pre-existing queue entry must be removed explicitly before enabling the guard; the long-term queue architecture remains part of #1058.
...
Protective retraction is not merge authorization. The trusted approval-independence job may use its write-scoped `github.token` only with `gh pr merge --disable-auto`, after a live read proves that an auto-merge request exists and the native PR author equals the governing `author_identity`; it immediately verifies the request is absent. That bounded path cannot arm, enqueue, or merge a PR, does not depend on the optional `AUTHOR_MERGE_TOKEN`, and is self-contained so the first workflow-plus-helper propagation PR cannot call a new helper option from an older trusted default branch.
