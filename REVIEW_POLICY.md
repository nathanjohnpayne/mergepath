...
The CodeRabbit column is `coderabbit_tier_of` (`scripts/lib/feedback-policy-helpers.sh`), the ladder both merge-relevant surfaces read since #884 routed them through one classifier: `scripts/coderabbit-severity-gate.sh` (the required gate) and `scripts/coderabbit-wait.sh`'s advisory count. CodeRabbit findings are keyed off the severity badge, top out at `p1` (Critical / Major / Potential issue / ⚠️), and never map to `p0` — `p0` is Codex-only. The daily rollup's own badge parser, `classify_severity` (`scripts/lib/daily-feedback-rollup-helpers.sh`), covers the badge rungs but not the two shapes #888 added, which it still returns `Unknown` for; `Unknown` routes to the `substantive` track, so the rollup surfaces such a finding rather than dropping it, but the two parsers are not one implementation — converging them is [#938](https://github.com/nathanjohnpayne/mergepath/issues/938). `🔴 Critical` joins the `p1` rung rather than opening a CodeRabbit `p0` (#888): `resolve_required_tiers` yields `{p1}` when a repo has no `feedback_policy` block, so a CodeRabbit `p0` would leave the top severity non-blocking exactly where a Major already blocks. A Refactor suggestion or a stray "security" mention carries no severity badge, so it is unclassified (discretionary) — unless the body carries CodeRabbit's machine-generated `cr-indicator-types:potential_issue` tag, which grades `p1` as a last-resort fallback when no badge is present at all (#888). The tag is a fallback, never an override: a badged body keeps its badge's tier, because the tag names the finding's category and the badge names its severity. That ordering is defined over one FINDING, not over one string, and CodeRabbit renders a finding's badge and its tag on different lines — so the surfaces that grade a multi-finding document line by line (this gate's PR-level summary loop, and `coderabbit-wait.sh`'s summary scan) go through `coderabbit_scan_tiers`, which scopes the tag rung to the stanza it trails. Grading each line independently instead made a `🟡 Minor` summary finding block as `[P1]` on the required gate, with no review thread to resolve it through (#936).
