...
- **`Self-Review Required`** — validates the PR body against the identity contract, via the trusted `scripts/validate-pr-body.sh` loaded from the default branch (Dependabot-exempt). It fails when the body lacks a `## Self-Review` section, or when it does not carry exactly one `Authoring-Agent:` naming a declared identity. **Every PR must name its writer**, and the set of writers is closed: the union of `author_identity`, `available_reviewers` and `non_reviewer_identities`, each accepted as its full login or its prefix-stripped short form (`nathanpayne-claude` or `claude`; `nathanjohnpayne` has no prefix and is accepted as itself). Adding an identity to any of those keys extends the set with no code change. Detection is markdown-aware: a `## Self-Review` heading inside a fenced code block does **not** satisfy it, which a line-based `grep` previously did (#1132). The policy consulted is the one governing the PR's **base** branch, resolved through `scripts/workflow/resolve_base_policy.sh`, not this workflow's own checkout. Rollout ordering, load-bearing: `scripts/validate-pr-body.sh` must already be on a repository's default branch before that repository receives this workflow — the gate deliberately loads the validator from the default branch, so it cannot be satisfied by a validator arriving in the same pull request. A direct-push sync lands both in one commit and is unaffected; a PR-based propagation must land the validator first.
