...
First run `scripts/review-feedback-accounting.sh <PR#> [owner/repo]` and require exit `0` with `posted == accounted`. This is independent of the thread gate below: a resolved thread can still be unread, and a top-level review-body finding has no thread to query. Then run the review-thread checks below.
