...
**Prompt gate re-evaluation (#658).** The lane's head-pinned `mergepath-propagation-lane verified-head=<sha>` marker (a `github-actions[bot]` issue comment) is also the signal the `Merge clearance gate` required check reads to exempt a verified propagation head (`scripts/merge-clearance-gate.sh` `lane_verified()`). Because that marker arrives as an issue comment — which fires no `pull_request` / review event — `merge-clearance-gate.yml` also triggers on `issue_comment`, job-guarded to the marker (`github-actions[bot]` author + the head-pinned marker body, on a PR), and posts a fresh `check_run` to the PR head. So a verified sync PR clears the gate promptly when the marker lands, instead of riding a fail-closed spurious-red until the 15-minute scheduled sweep resolves it (before #657's triage change, the `needs-external-review` add/remove dance produced an `unlabeled` event that re-ran the gate; once triage stopped adding the label, nothing re-triggered it). The `issue_comment` guard is only a cheap pre-filter; the marker's trust is unchanged — head-pinned and `github-actions[bot]`-authored, re-verified in `lane_verified()` against the current head — and label events remain untrusted (they are not head-pinned proof).
