...
15a. The loop continues until one of the following terminates it:
     ...
     - **Clearance (happy path).** Codex posts a review with no unaddressed **`required`-tier** inline findings (P0/P1 by default; see [§ Feedback Disposition Policy](#feedback-disposition-policy)) on the current HEAD, OR reacts 👍 on or after the current HEAD commit, OR posts a HEAD-anchored affirmative issue-comment verdict (`Didn't find any major issues` + a `Reviewed commit: <sha>` line whose sha prefixes HEAD) with no unaddressed required-tier findings on HEAD (#600/#567). Proceed to step 16a. **Note on a verdict-only response:** `codex-review-request.sh`'s poll does **not** yet key off the issue-comment verdict (it scans reviews/reactions; see step 12a and #609), so a verdict-only Codex response reaches the poll's timeout (exit `4`) — at which point the agent still proceeds to the step-16a merge-gate check (`codex-review-check.sh`), which **does** honor the HEAD-anchored verdict as of #600. The verdict-only clearance is therefore a **merge-gate** signal, not a request-loop terminator.
