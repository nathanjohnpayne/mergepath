...
16a. Before merging, the agent runs `scripts/codex-review-check.sh <PR#>` to verify the merge gate. All of the following must be true:
     ...
     - **Gate (a)** — every REQUIRED status check on the PR's base branch is green on the current head. "Required" is resolved by `scripts/lib/branch-requirements.sh` (#1064, subsuming #1063), which reads the two rule surfaces a plain write-scoped token can see — GraphQL `refUpdateRule.requiredStatusCheckContexts` for classic branch protection, and REST `rules/branches/{branch}` for rulesets — and returns their union. Both are read because GitHub enforces classic protection and every applicable ruleset simultaneously, and neither surface reports the other. Checks outside that list, including a consumer's `repo_lint_local.yml` annex, do not block gate (a) by name; the annex is enforced separately by workflow identity (#655). Each required NAME is collapsed to its current run before being judged — a gate that republishes under one name leaves superseded failures in the rollup, and GitHub itself resolves the context to its latest run — so a stale red run does not block a PR that GitHub would merge.
       ...
       The resolver is tri-state and the three states are kept distinct: `known` with a list (filter to it), `known` with an empty list (an approvals-only branch genuinely requires nothing, so no filter applies), and `unknown` (no surface answered — fail closed per [#465](https://github.com/nathanjohnpayne/mergepath/issues/465), requiring every non-skipped rollup check to be green). Conflating the last two is the defect [#1064](https://github.com/nathanjohnpayne/mergepath/issues/1064) closed. `unknown` now means both viewer-readable surfaces failed, which is an API or network fault rather than a permission shape, and a fault of that class would normally have already exited 3 at the retry-wrapped rollup read.
     - **Gate (b)** — one of:
