...
**Which `review-policy.yml` is evaluated — the PR's BASE branch, always (#769).** The policy that governs a PR is the policy of the branch it targets, not the default branch. Five surfaces read it, and none of them may read it from the PR's own checkout. Four — `agent-review.yml`'s triage job, `agent-review.yml`'s `load-config` job (through `scripts/workflow/load_review_config.sh`, #788), `scripts/merge-clearance-gate.sh`, and `scripts/codex-review-check.sh` (which the auto-clear workflow also runs) — resolve it through the single shared helper `scripts/workflow/resolve_base_policy.sh`. `load-config` is the one whose outputs are not only about requiredness: it exports `available_reviewers` and `author_identity` to the reviewer-assignment step and to the auto-merge arming gate, so reading them from any other branch could assign a reviewer the base policy does not list, or arm auto-merge from an identity it does not permit. The fifth, `pr-review-policy.yml`, satisfies the same contract **structurally rather than by calling the helper**: it materializes the PR base commit as a git worktree and reads the policy from there, which *is* the base policy by construction and needs no API call. Its fallback, when the base predates the policy file, comes from a trusted `origin/<default>` worktree — never a bare path, which would resolve against the PR merge commit and let a PR supply the policy that judges it. Between them the gate-enable switch, the threshold, the protected-path list, the reviewer allow-list, and the Phase 4b-substitute rules can never come from different branches. The two implementations are equivalent by contract, not by shared code, so a change to one must be mirrored in the other. Its contract rests on one rule: **fall back to the default-branch policy only when the base is POSITIVELY PROVEN to be the default branch.** A PR onto the default branch therefore uses the checked-out policy and makes **no** API call (the two are the same file, so the fleet's ordinary path gains no new dependency); a **non-default** base has its policy fetched at the base sha; a **verified** missing-file 404 falls back, since no stricter policy can exist to miss — verified meaning gh reported `HTTP 404` *and* the base commit itself resolves, so an unreadable repo or ref (which returns the same status) cannot masquerade as an absent policy; and everything else — an unreadable non-default policy, an undeterminable base, or a missing resolver — **fails closed**. "Unknown" is not proof.
