...
**Which `review-policy.yml` is evaluated — the PR's BASE branch, always (#769).** The policy that governs a PR is the policy of the branch it targets, not the default branch. Four surfaces decide requiredness — `pr-review-policy.yml`, `agent-review.yml`'s triage job, `scripts/merge-clearance-gate.sh`, and `scripts/codex-review-check.sh` (which the auto-clear workflow also runs) — and all four resolve it through the single shared helper `scripts/workflow/resolve_base_policy.sh`, so the gate-enable switch, the threshold, the protected-path list, the reviewer allow-list, and the Phase 4b-substitute rules can never come from different branches. Its contract: a PR onto the **default branch** uses the checked-out policy and makes **no** API call (the two are the same file, so the fleet's ordinary path gains no new dependency); a **non-default** base has its policy fetched at the base sha; a confirmed **404** (a base predating the policy file) falls back to the default-branch copy; and any **other** read failure on a positively-identified non-default base is fail-closed. An *undeterminable* base is deliberately **not** fail-closed — that would let a transient API error block merges fleet-wide, and it is not an exploitable downgrade since nobody can choose to make their own PR metadata unreadable.
