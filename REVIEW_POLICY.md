...
**Successful review-trigger acknowledgements in polling (#1386).** Polling selection treats a supported provider-owned `Actions performed` / `Review triggered` acknowledgement (including `Full review triggered`) as a new run in progress. It retains the newest acknowledgement above older clean or benign publications; a newer substantive publication is selected normally. The next older substantive pause or rate-limit notice still wins, preserving its recovery and exit routing. Ordinary polling, StatusContext arbitration, and the post-probe terminal upgrade consume that classification. Anchor-free refusal/window selectors continue skipping these acknowledgements as narration, existing exact-head review and `--probe` contracts remain unchanged, and an acknowledgement supplies no review evidence. Quoted/fenced lookalikes remain content; an unreadable acknowledgement cannot clear the head.
