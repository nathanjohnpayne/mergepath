...
**GitHub Advanced Security / code scanning (#1101).** A `github-advanced-security[bot]` inline comment (the form GitHub's CodeQL analyzer posts) carries no severity in its body — only a link to the alert (`.../security/code-scanning/<number>`). `scripts/review-feedback-accounting.sh` resolves the tier by reading `repos/{owner}/{repo}/code-scanning/alerts?ref=refs/pull/{pr}/head` and mapping the matching alert's `rule.security_severity_level` through `ghas_severity_tier` (`scripts/lib/feedback-policy-helpers.sh`) onto the same ladder. That fetch is lazy — it runs only when at least one `github-advanced-security[bot]` inline comment is present on the PR — so a repo that has never enabled code scanning pays no extra API call and cannot be broken by this feature. The `ref` parameter is required: the unscoped list endpoint does not reliably include an alert that exists only on the PR branch, confirmed live against nathanjohnpayne/nathanpaynedotcom#809. An alert whose severity cannot be resolved (the alert number can't be parsed from the comment, the alert isn't in the fetched set, or the rule carries no `security_severity_level` at all — a non-security CodeQL quality query) maps to `p2`: tracked and accountable, but not merge-blocking under the default `feedback_policy` below, rather than silently dropped. This closes the *accounting* gap only. It does not add a dedicated merge-blocking required CI check (a `code-scanning-severity-gate.sh` twin of `codex-p1-gate.sh` / `coderabbit-severity-gate.sh`); that is tracked as a narrow-start follow-up.
