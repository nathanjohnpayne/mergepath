...
The `block-self-approval` job in `.github/workflows/agent-review.yml` enforces both halves of this boundary (#1094). It always blocks an allow-listed agent reviewer from approving a PR whose native GitHub author is that same reviewer account. Triage resolves the policy governing the PR base, recomputes applicability from the complete paginated file list, and exports the pre-clearance Phase 4 result; a prior-review carry-forward does not erase Phase 4 scope, while an exact-head byte-verified propagation-lane PR retains its documented under-threshold-equivalent standing. For a PR under the configured shared `author_identity`, Phase 3 otherwise allows the authoring agent's reviewer identity, while Phase 4 requires exactly one well-formed `Authoring-Agent:` declaration that maps to an allow-listed reviewer other than the approver. A missing, malformed, duplicate, unknown, or indeterminate declaration fails closed, with the dismissal reason naming the exact declaration defect. The guard applies that decision both to a new approval and to every synchronize, reopen, or blocking-label-removal re-arm: it collapses the complete paginated review history to each registered reviewer's latest opinionated state, dismisses approvals that became ineligible after the PR crossed into Phase 4, and exposes an eligible-approval output that every auto-merge trigger must consume. A confirmed native-account or same-authoring-agent violation adds the persistent, human-remove-only `policy-violation` label. An indeterminate declaration and a carried approval made ineligible by a later transition are dismissed without that label, so repairing the body and submitting a fresh independent approval is sufficient recovery. This transition has no approval grace: the pull request template and generated propagation PRs supply the declaration on normal paths, while the guarded author workflow requires the marker and Phase 1 requires a populated line. The canonical workflow carries a bounded strict bootstrap for the first atomic workflow/module rollout only; the adversarial suite extracts that bootstrap from the workflow and requires decision parity with `scripts/self-approval-detector.cjs`, and a trusted workflow that already carries the detector-required marker fails closed if the module is missing. Before enabling the guard in a consumer, the canary rollout audits open shared-author Phase 4 PR bodies and repairs any invalid declaration. Allowing an unverifiable approval during rollout would preserve the #1094 hole. A PR whose native author is not the shared identity does not require this synthetic metadata because the author/reviewer separation is native; an unregistered human reviewer remains eligible as the documented tiebreaker. This is a review-time independence check, not authentication of the PR-body claim: declaration provenance and spoof resistance remain the separate #928 workstream.
