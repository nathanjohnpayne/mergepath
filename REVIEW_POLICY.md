...
- **Change level.** When a PR reverses direction — it abandons its own approach, implements and then removes something, materially narrows or widens scope after review, or is arbitrated by the human tiebreaker — the PR body carries a `## Path taken` section recording the chronology, what survived versus what was discarded and why, and who arbitrated; and struck acceptance criteria stay visible as `~~strikethrough~~` rather than being deleted. The record goes in the body, never the title — a title describes the change's end state, not the session that produced it. **Fixing a review finding and pushing again is not a reversal** — the trigger is a change of direction, not a change of code, and the doc's non-trigger list is as binding as its trigger list.
