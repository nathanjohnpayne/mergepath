...
For non-Dependabot PRs, the completion and scheduled continuation paths independently revalidate the exact governing policy, author-token identity, required checks, feedback accounting, conversation state, and complete latest approval set. They compare a post-independence authorization tuple containing state, draft status, head, base ref/SHA, native author, labels, and body. GitHub exposes a head precondition for merge mutations but no base-SHA precondition, so finite reads cannot bind a same-head base-policy transition. Every such continuation path performs workflow-token arm cleanup with exact absent-state readback. A classified native non-shared path may then report stable readiness for an ordinary one-shot author merge; a shared-author path deliberately stops after cleanup and remains on the manual author workflow. No path enables auto-merge, directly merges, or enqueues a non-Dependabot PR. Unattended merging remains deferred to #1058's merge-group boundary. The canonical workflow keeps a generated strict bootstrap for the first atomic workflow/module rollout, and canary rollout requires arm/queue audits plus `human-hold`. Declaration provenance and spoof resistance remain #928; the exact `dependabot[bot]` login short-circuits the general continuation after its first valid live read and remains governed by its dedicated workflow.

The approval guard's retraction is intentionally self-contained in the trusted workflow and uses only the workflow's write-scoped token, so it works when `AUTHOR_MERGE_TOKEN` is absent and during the first consumer PR before the new continuation helper exists on that consumer's default branch. Every existing non-Dependabot durable arm is unsafe at this boundary because GitHub provides no base-SHA precondition for an eventual merge; the guard can therefore issue only `--disable-auto` followed by exact-head absent-state readback, regardless of whether the native author is shared. Dependabot's exact native login remains outside this general continuation path because its dedicated workflow owns that durable lane. The legacy `auto_arm_allowed` output now means only that a native non-shared PR may enter the privileged readiness-validation job; it is not merge authorization and that job never arms, enqueues, or merges. Every shipping caller that may proceed to normal continuation first runs the same bounded protective pass with `github.token`, then invokes normal continuation with the author token plus a separately supplied `MERGEPATH_PROTECTIVE_TOKEN`; the auto-clear label-removal paths invoke only that protective pass before their PAT-backed deletion. Normal continuation installs cleanup before any fallible readiness work, re-resolves the governing policy, verifies the author token against its `author_identity`, and retracts any non-Dependabot arm that appears between reads or before any exit; cleanup ambiguity fails closed. The trusted default-branch policy is helper provenance, not identity authority for a divergent non-default base. Rollout requires `human-hold` on the hub PR and every first-wave consumer propagation PR, plus temporary removal or withholding of the optional `AUTHOR_MERGE_TOKEN` Actions secret before the rollout PR is opened; the missing credential is the mechanical version-skew barrier that prevents an older trusted default-branch continuation from re-arming while required checks remain unchanged. Before opening each rollout PR, audit every open agent-review PR for both an `autoMergeRequest` and a `mergeQueueEntry`, disable each request, and explicitly dequeue each queue entry: GitHub CLI cannot dequeue an already queued PR through `--disable-auto`. After all required checks and reviews pass, repeat both audits, have the human remove `human-hold`, and perform a normal one-shot author merge. Verify the new workflow marker and helper on the default branch and repeat the no-arm/no-queue audit before restoring the optional secret. The long-term queue architecture remains part of #1058.
...
The non-Dependabot `auto-merge-on-approval` job name is retained for workflow and consumer compatibility, but the job no longer arms auto-merge, enqueues, or performs a merge. When the repo has an `AUTHOR_MERGE_TOKEN` Actions secret, it verifies that the token resolves to the governing `author_identity` and evaluates stable readiness; when the secret is absent, it stops after workflow-token cleanup. Reviewer tokens such as `REVIEWER_ASSIGNMENT_TOKEN` must not be used for PR merges. Unattended non-Dependabot merging remains disabled pending #1058.

Protective retraction is not merge authorization. The trusted approval-independence job may use its write-scoped `github.token` only with `gh pr merge --disable-auto` after a live read proves that a non-Dependabot auto-merge request exists; it immediately verifies the same native author and exact head with an absent request. It retracts every non-Dependabot durable arm, including a native non-shared one, because an eventual GitHub auto-merge cannot be bound to the reviewed base SHA. Dependabot retains its separately governed durable arm. That bounded path cannot arm, enqueue, or merge a PR, does not depend on the optional `AUTHOR_MERGE_TOKEN`, and is self-contained so the first workflow-plus-helper propagation PR cannot call a new helper option from an older trusted default branch.

Readiness validation is event-driven rather than runner-polled. Three triggers re-evaluate it: the original `pull_request_review` + `approved` event; a `pull_request` `synchronize` / `reopened` event; and a blocking-label `unlabeled` settle event. Every path first proves a registered non-author latest-state `APPROVED` review, verifies the author token, re-reads the shared blocking-label set, and probes current-head required-check readiness once. CodeRabbit is not polled and is not an additional requirement after that approval; required-severity findings and unresolved conversations remain enforced by their dedicated required checks. If checks are pending, the job exits without holding a runner. Successful `repo-lint`, optional local-annex, and policy workflow completions re-enter the trusted default-branch continuation; the five-minute sweep is the general backstop for consumer annexes whose custom display name cannot be enumerated by `workflow_run`. The continuation resolves the live PR by exact head, reruns `codex-review-check.sh`, requires feedback accounting and zero unresolved conversations, revalidates the full authorization tuple after approval independence, and retracts any late durable arm with the workflow token. A classified native non-shared path reports stable readiness without invoking a merge mutation; a shared-author path stops after cleanup for the ordinary one-shot author workflow. A push with no existing approval does not become ready, and a withdrawn approval does not count. Both the approval job and continuation source helpers only from the trusted default branch. During the first atomic workflow-plus-helper rollout, the approval job uses an exact inline blocking-label fallback until the trusted default branch contains the helper; later runs source the shared helper.
...
**Automated merge identity.** Non-Dependabot unattended merging is disabled pending #1058's merge-group boundary; this is not a sanctioned reviewer-identity exception. The readiness job may use an author-owned `AUTHOR_MERGE_TOKEN` Actions secret only after verifying that it resolves to the `author_identity` in the policy **governing the PR** — the base branch's. `agent-review.yml` resolves that identity in `load-config` through `scripts/workflow/resolve_base_policy.sh` for its event-bound gates, while `scripts/workflow/approval-merge-continuation.sh` independently re-resolves the exact live governing policy before author-token verification. Never use the copy in the job's own checkout as identity authority: that checkout may be the PR's merge ref on a `pull_request` event, so a PR could nominate its own authority, and on a non-default base the default-branch copy is wrong in the other direction. An identity the governing policy does not prove is empty — whether because the policy could not be read or names no top-level `author_identity` — and an empty one disables readiness continuation rather than falling back to a hard-coded login. The author token never licenses the continuation to arm, enqueue, or merge; ordinary merges remain one-shot through `scripts/gh-as-author.sh` after all gates and holds clear.
