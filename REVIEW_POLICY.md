...
The `block-self-approval` job in `.github/workflows/agent-review.yml` enforces both halves of this boundary (#1094). It always blocks an allow-listed agent reviewer from approving a PR whose native GitHub author is that same reviewer account. Triage resolves the policy governing the PR base, recomputes applicability from the complete paginated file list, and exports the pre-clearance Phase 4 result; a prior-review carry-forward does not erase Phase 4 scope, while an exact-head byte-verified propagation-lane PR retains its documented under-threshold-equivalent standing. For a PR under the configured shared `author_identity`, Phase 3 otherwise allows the authoring agent's reviewer identity, while Phase 4 requires exactly one well-formed `Authoring-Agent:` declaration that maps to an allow-listed reviewer other than the approver. A missing, malformed, duplicate, unknown, or indeterminate declaration fails closed, with the dismissal reason naming the exact declaration defect. The guard applies that decision both to a new approval and to every synchronize, reopen, PR-edit, or blocking-label-removal re-evaluation: it collapses the complete paginated review history to each registered reviewer's latest opinionated state, dismisses approvals that became ineligible after the PR crossed into Phase 4 or its mutable body/base changed, and exposes an eligible-approval output that every auto-merge trigger must consume. An edit invalidates only; it does not itself arm auto-merge. Agent Review runs are serialized per PR, but only a successor that reruns equivalent enforcement cancels an older event snapshot; COMMENTED or CHANGES_REQUESTED reviews, opened/ready events, labeled events, and unrelated label removals preserve their producer. Because the completion and scheduled continuation paths sit outside that workflow concurrency boundary, `scripts/workflow/approval-merge-continuation.sh` also invokes a final read-only check immediately before its exact-head merge write: it pins the head and base ref/SHA, derives live Phase 4 scope against that exact base independently of the optional external-gate enablement knob, materializes the same base policy and re-verifies the merge identity, and brackets complete paginated review reads with stable live PR body/state/base/label snapshots before running the canonical detector. Any opinionated-review or mutable-metadata change defers. The final set must contain an independent current-head approval and no standing disallowed same-agent approval on any head; a malformed review object, duplicate review ID, non-canonical review timestamp, malformed label array, unreadable policy, indeterminate requiredness, review-history error, PR-body error, or detector error fails closed as infrastructure. No PR whose native author is the shared `author_identity` may retain a GitHub auto-merge request: the head precondition cannot atomically bind the mutable body, base policy, labels, review state, or later Phase 4 transition, so an arm created while the PR was under threshold could survive after the same head becomes Phase 4. The continuation retracts any existing request before every readiness exit, and the approval guard invokes the same shared-author-only retraction even when the changed approval is no longer eligible to reach the arming job. The approval itself remains valid under Phase 3; only the merge becomes a one-shot author-identity action after every live gate is clean and any task-specific hold is released. Native non-shared authors retain automated arming. Restoring unattended arming for the shared-author class requires a GitHub-native immutable boundary for all governing mutable state, tracked separately in #928. A confirmed native-account or same-authoring-agent violation adds the persistent, human-remove-only `policy-violation` label. An indeterminate declaration and a carried approval made ineligible by a later transition are dismissed without that label, so repairing the body and submitting a fresh independent approval is sufficient recovery. This transition has no approval grace: the pull request template and generated propagation PRs supply the declaration on normal paths, while the guarded author workflow requires the marker and Phase 1 requires a populated line. The canonical workflow carries a bounded strict bootstrap for the first atomic workflow/module rollout only. `scripts/render-self-approval-bootstrap.cjs` generates that mirror from the canonical detector source, the CI wrapper requires the generated bytes to remain current, the adversarial suite exercises both implementations plus all three module-selection states, and a trusted workflow that already carries the detector-required marker fails closed if the module is missing. Before enabling the guard in a consumer, the canary rollout audits every open shared-author PR, repairs any invalid Phase 4 declaration, and disables every existing shared-author auto-merge request. Allowing an unverifiable or already-armed approval during rollout would preserve the #1094 hole. An unregistered human reviewer remains eligible as the documented tiebreaker. Dependabot bypasses this agent-only job and remains governed by its dedicated approval workflow. This is a review-time independence check, not authentication of the PR-body claim: declaration provenance and spoof resistance remain the separate #928 workstream.

The approval guard's retraction is intentionally self-contained in the trusted workflow and uses only the workflow's write-scoped token, so it works when `AUTHOR_MERGE_TOKEN` is absent and during the first consumer PR before the new continuation helper exists on that consumer's default branch. It classifies the live native author against the governing identity, leaves a proven native non-shared arm untouched, and can issue only `--disable-auto` followed by readback; if an existing arm cannot be classified because policy identity is unavailable, safety wins and the unclassified arm is retracted. The guard exports a live `auto_arm_allowed` result that the later privileged job must consume, so a pre-rollout trusted helper cannot re-arm a shared or unclassified PR later in the same run. The helper's steady-state path independently verifies its author token against the trusted default-branch policy before any protective write, then still resolves and verifies the governing base policy before any arm. Rollout requires `human-hold` on the hub PR and every first-wave consumer propagation PR, plus temporary removal or withholding of the optional `AUTHOR_MERGE_TOKEN` Actions secret before the rollout PR is opened; the missing credential is the mechanical version-skew barrier that prevents the consumer's old default-branch Agent Review or auto-clear continuation from re-arming, while required checks remain unchanged. Before opening each rollout PR, audit every open shared-author PR for both an `autoMergeRequest` and a `mergeQueueEntry`, disable each request, and explicitly dequeue each queue entry: GitHub CLI cannot dequeue an already queued PR through `--disable-auto`. After all required checks and reviews pass, repeat both audits, have the human remove `human-hold`, and perform a normal one-shot author merge. Verify the new workflow marker and helper on the default branch and repeat the no-arm/no-queue audit before restoring the optional secret. The long-term queue architecture remains part of #1058.
...
The non-Dependabot `auto-merge-on-approval` workflow is opt-in only: it may arm auto-merge or perform a merge only when the repo has an `AUTHOR_MERGE_TOKEN` Actions secret and that token resolves to `author_identity` (normally `nathanjohnpayne`). If the secret is absent, the workflow stops after validation and leaves manual author merge as the default. Reviewer tokens such as `REVIEWER_ASSIGNMENT_TOKEN` must not be used for PR merges.

Protective retraction is not merge authorization. The trusted approval-independence job may use its write-scoped `github.token` only with `gh pr merge --disable-auto` after a live read proves that an auto-merge request exists and either the native PR author equals the governing `author_identity` or the arm cannot be classified because that identity is unavailable; it immediately verifies the same native author and an absent request. That bounded path cannot arm, enqueue, or merge a PR, does not depend on the optional `AUTHOR_MERGE_TOKEN`, and is self-contained so the first workflow-plus-helper propagation PR cannot call a new helper option from an older trusted default branch. A proven native non-shared arm is left untouched.

Arming is not one-shot on the approval event. The approval job arms on three triggers: the original `pull_request_review` + `approved` event; a `pull_request` `synchronize` / `reopened` re-arm; and a blocking-label `unlabeled` settle re-arm. Every path first proves a registered non-author latest-state `APPROVED` review, verifies the author merge token, re-reads the shared blocking-label set, and probes current-head required-check readiness once. CodeRabbit is not polled and is not an additional requirement after that approval; required-severity findings and unresolved conversations remain enforced by their dedicated required checks. If checks are pending, the approval job exits without holding a runner. Successful `repo-lint`, optional local-annex, and policy workflow completions re-enter the trusted default-branch continuation; the five-minute sweep is the general backstop for consumer annexes whose custom display name cannot be enumerated by `workflow_run`. The continuation resolves the live PR by exact head, reruns `codex-review-check.sh`, requires feedback accounting and zero unresolved conversations, re-reads state, head, draft status, and blocking labels, then calls `gh pr merge --auto --match-head-commit`. A push with no existing approval does not arm, and a withdrawn approval does not count. Both the approval job and continuation source merge helpers only from the trusted default branch. During the first atomic workflow-plus-helper rollout, the approval job uses an exact inline blocking-label fallback until the trusted default branch contains the helper; later runs source the shared helper.
...
**Automated merge identity.** Non-Dependabot automatic PR merge is not a sanctioned reviewer-identity exception. If enabled, it must use an author-owned `AUTHOR_MERGE_TOKEN` Actions secret and verify the token resolves to `author_identity` immediately before the merge path. The `author_identity` it verifies against is the one in the policy **governing the PR** — the base branch's, resolved once in `agent-review.yml`'s `load-config` job through `scripts/workflow/resolve_base_policy.sh` and consumed from that job's output (#788). Never the copy in the merge job's own checkout: that checkout is the PR's merge ref on a `pull_request` event, so a PR could nominate the identity permitted to merge it by editing policy on its own branch, and on a non-default base the default-branch copy is wrong in the other direction. An identity the governing policy does not prove is empty — whether because the policy could not be read at all, or because it was read and names no top-level `author_identity` — and an empty one disables automatic merge rather than falling back to a hard-coded login, at either end of the handoff. Repos without that secret keep the normal manual merge flow under `scripts/gh-as-author.sh`.
...
**Which `review-policy.yml` is evaluated — the PR's BASE branch, always (#769).** The policy that governs a PR is the policy of the branch it targets, not the default branch. Six surfaces read it, and none of them may read it from the PR's own checkout. Five — `agent-review.yml`'s triage job, `agent-review.yml`'s `load-config` job (through `scripts/workflow/load_review_config.sh`, #788), `scripts/merge-clearance-gate.sh`, `scripts/codex-review-check.sh` (which the auto-clear workflow also runs), and `scripts/review-feedback-accounting.sh` — resolve it through the single shared helper `scripts/workflow/resolve_base_policy.sh`. `load-config` is the one whose outputs are not only about requiredness: it exports `available_reviewers` and `author_identity` to the reviewer-assignment step and to the auto-merge arming gate, so reading them from any other branch could assign a reviewer the base policy does not list, or arm auto-merge from an identity it does not permit. The accounting caller always passes `--materialize-default`, so it materializes the target repository's policy at the exact PR-base SHA even for a same-repository PR targeting the default branch; this also lets trusted cross-repository Phase 4b orchestration avoid substituting the orchestrator checkout's copy. The sixth surface, `pr-review-policy.yml`, satisfies the same contract **structurally rather than by calling the helper**: it materializes the PR base commit as a git worktree and reads the policy from there, which *is* the base policy by construction and needs no API call. Its fallback, when the base predates the policy file, comes from a trusted `origin/<default>` worktree — never a bare path, which would resolve against the PR merge commit and let a PR supply the policy that judges it. Between them the gate-enable switch, the threshold, the protected-path list, the reviewer allow-list, and the Phase 4b-substitute rules can never come from different branches. The two implementations are equivalent by contract, not by shared code, so a change to one must be mirrored in the other. Its contract rests on one rule: **fall back to the default-branch policy only when the base is POSITIVELY PROVEN to be the default branch.** A PR onto the default branch therefore uses the checked-out policy and makes **no** contents API call when the caller is in that same repository, except that `scripts/review-feedback-accounting.sh` deliberately materializes the exact base-SHA policy even on that same-repository default-base path; a cross-repository caller likewise materializes the target policy at the base SHA; a **non-default** base has its policy fetched at the base SHA; a **verified** missing-file 404 falls back, since no stricter policy can exist to miss — verified meaning gh reported `HTTP 404` *and* the base commit itself resolves, so an unreadable repo or ref (which returns the same status) cannot masquerade as an absent policy; and everything else — an unreadable non-default policy, an undeterminable base, or a missing resolver — **fails closed**. "Unknown" is not proof.
