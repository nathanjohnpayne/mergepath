...
#   13. by-priority p0+p1 required → unresolved ⚠️ Critical (p0) blocks → exit 1.
...
# ---------------------------------------------------------------------------
# Test 13: by-priority p0+p1 required → unresolved ⚠️ Critical (p0) blocks.
# ---------------------------------------------------------------------------
echo
echo "--- Test 13: p0 required → unresolved ⚠️ Critical blocks"
SCRATCH=$(make_scratch_with_policy "$DEFAULT_POLICY")
FIXTURE_PR=$(make_pr_fixture "$HEAD_SHA")
...
if [ "$RC" = 1 ] && echo "$OUT" | grep -q "CodeRabbit blocking-tier unresolved: 1" \
  ...
  pass "p0 required → unresolved ⚠️ Critical → exit 1, listed as [P0]"
else
