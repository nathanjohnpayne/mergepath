...
# the two required contexts pr-review-policy.yml publishes
...
#   5.  Self-Review: a body with no `## Self-Review` → failure; 5b a body with
#       one → success. Routed through scripts/validate-pr-body.sh, so a
#       heading inside a fenced code block does not count (5c).
#   6.  Label Gate: a blocking label → failure naming it; 6b no labels →
#       success; 6c a non-blocking label → success.
#   7.  Dependabot PR → Self-Review published as `skipped` (the event-driven
#       job's exemption), while Label Gate still evaluates.
...
#   11. A PR-detail read fails → that PR is skipped, exit 1, other PRs still
#       swept.
...
case "$endpoint" in
  ...
  *"/check-runs?check_name="*)
    ...
    fail_var="FAIL_CHECKRUNS_${name//-/_}"
    [ -z "${!fail_var:-}" ] || { echo "gh: HTTP 502 (check-runs)" >&2; printf '{"message":"Bad gateway"}\n'; exit 1; }
    f="FIXTURE_CHECKRUNS_${name//-/_}_${sha}"
    src="${!f:-}"
    if [ -z "$src" ]; then
      src="$GH_STUB_EMPTY_CHECKRUNS"
    fi
    emit "$src"
    exit 0
    ;;
...
write_open_prs() {  # <number>...
  if [ $# -eq 0 ]; then
    printf '[]\n' > "$WORKDIR/open-prs.json"
  else
    printf '%s\n' "$@" | jq -R 'tonumber' | jq -s 'map({number: .})' > "$WORKDIR/open-prs.json"
  fi
...
write_check_runs() {  # <context-slug> <sha> <external_id>...
  ...
  local path="$WORKDIR/check-runs-$slug-$sha.json"
  local key
  # Same normalisation the stub applies when it looks the fixture back up.
  key=$(printf '%s' "$sha" | tr -c 'A-Za-z0-9' '_')
  printf '%s\n' "$@" | jq -R . | jq -s 'map({external_id: .}) | {total_count: length, check_runs: .}' > "$path"
...
run_sweep() {
  ...
  : > "$GH_WRITES"
  : > "$GH_CALLS_LOG"
  export GH_WRITES GH_CALLS_LOG
  RC=0
  OUT=$(PATH="$STUB_DIR:$PATH" "$SCRIPT" "$REPO" 2>&1) || RC=$?
  WRITES=$(cat "$GH_WRITES")
}

published_conclusion() {  # <context> — prints the conclusion published for it
...
# 6b. Label Gate success with no labels.
# ---------------------------------------------------------------------------
reset_env
write_open_prs 7
...
#     the lane routes through scripts/validate-pr-body.sh rather than grepping.
# ---------------------------------------------------------------------------
reset_env
write_open_prs 7
...
# ---------------------------------------------------------------------------
# 4. A run THIS lane published → refreshed, so a stale recovery verdict cannot
...
# ---------------------------------------------------------------------------
# 11b. A PR whose head sha does not read as a git object name publishes
#      nothing: a verdict cannot be pinned to a head that was never resolved.
# ---------------------------------------------------------------------------
reset_env
write_open_prs 7
...
# ---------------------------------------------------------------------------
# 12. The lane's only mutating call is the check-run POST. It must never label,
#     comment, or otherwise write PR state — its job permissions grant
#     `pull-requests: read`, and a mutation added here would 403 in production
