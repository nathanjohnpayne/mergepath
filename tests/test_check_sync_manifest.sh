...
if [ -f "$LIVE_MANIFEST" ] && [ -f "$LIVE_MARKER" ]; then
  ...
  # the old three. Case 63 pins the derivation in both directions.
...
FOPEN_EOF
  ...
  # Case 63: tell (b)'s hub-only doc set tracks the live manifest.
  #
  ...
  if [ -n "$hub_unmatched" ]; then
    fail "Case 63: HUB_ONLY_RE does not match hub-only doc(s) the manifest declares (the alternation has drifted behind doc_ownership):$hub_unmatched"
  elif [ -n "$canon_false_pos" ]; then
    fail "Case 63: HUB_ONLY_RE matches canonical doc path(s) it must not:$canon_false_pos"
  else
    pass "Case 63: tell-(b) hub-only alternation covers all $hub_declared manifest-declared hub-only docs and no canonical sibling"
  fi
