...
# ---------------------------------------------------------------------------
# EPIPE hygiene guard (#1147/#1148)
#
# `printf … | grep -q` under pipefail reports a MATCH as a failure: grep -q
# exits on the match, printf takes EPIPE, pipefail propagates it. The guard
# must reject the shape -- INCLUDING when an intermediate filter sits between
# printf and the short-circuit grep, because the middle stage is what takes
# EPIPE there and a here-string on the front does not fix it.
# ---------------------------------------------------------------------------
epipe_case() {
  local name="$1" body="$2" expect="$3" why="$4"
  local repo; repo="$(make_scratch_repo "$name")"
  mk_workflow "$repo" "name: t
jobs:
  lint:
    steps:
      - name: check_ci_scripts_wired
        run: ./scripts/ci/check_ci_scripts_wired
      - name: check_probe
        run: ./scripts/ci/check_probe
"
  printf '#!/usr/bin/env bash\nset -euo pipefail\n%s\n' "$body" >"$repo/scripts/ci/check_probe"
  chmod +x "$repo/scripts/ci/check_probe"
  local out rc=0
  out="$(run_check "$repo" 2>&1)" || rc=$?
  if [[ "$expect" == "reject" ]]; then
    if [[ "$rc" -ne 0 ]] && grep -qF 'piped short-circuit grep' <<<"$out"; then
      pass "$why"
    else
      fail "$why (rc=$rc; output: ${out:0:200})"
    fi
  else
    if [[ "$rc" -eq 0 ]]; then
      pass "$why"
...
epipe_case epipe_direct \
  'printf "%s\\n" "$x" | grep -qF needle' \
  reject "the guard rejects printf piped straight into grep -q"

# The case this guard originally MISSED: an intermediate filter between the
# producer and the short-circuit consumer.
epipe_case epipe_intermediate \
  'printf "%s\\n" "$x" | grep -A6 ctx | grep -qF needle' \
  reject "the guard rejects a short-circuit grep behind an intermediate filter"

epipe_case epipe_flag_cluster \
  'printf "%s\\n" "$x" | grep -Fxq needle' \
  reject "the guard rejects clustered short-circuit flags (-Fxq)"

# grep -c consumes all of stdin, so it cannot take EPIPE and must NOT be flagged.
epipe_case epipe_count_ok \
  'n=$(printf "%s\\n" "$x" | grep -cF needle || true); echo "$n"' \
  accept "the guard does not flag grep -c, which cannot take EPIPE"

# The corrected shapes must pass.
epipe_case epipe_herestring_ok \
  'grep -qF needle <<<"$x"' \
  accept "the guard accepts the here-string form"

epipe_case epipe_captured_ok \
  'ctx="$(grep -A6 ctx <<<"$x" || true)"; grep -qF needle <<<"$ctx"' \
  accept "the guard accepts a captured intermediate followed by a here-string grep"
