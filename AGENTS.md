...
6. Before any merge attempt, query GitHub review-thread state, not just flat
   ...
   `resolveReviewThread` path directly, then list again. Never auto-resolve
   human-authored threads.
7. Check .github/review-policy.yml for the external review threshold. If the PR does NOT meet it (lines changed < external_review_threshold AND no file matches external_review_paths), merge as nathanjohnpayne after the pre-merge review conversation gate is clean. Done.
