...
6. Before any merge attempt, query GitHub review-thread state, not just flat
   ...
   unresolved review conversations. For bot-authored threads only, if the
   finding has been fixed on the current HEAD or rebutted on-thread, resolve
   via `scripts/resolve-pr-threads.sh <PR#> --auto-resolve-bots` or the
   identity-checked `resolveReviewThread` path, then list again. Never
   auto-resolve human-authored threads.
7. Check .github/review-policy.yml for the external review threshold. If the PR does NOT meet it (lines changed < external_review_threshold AND no file matches external_review_paths), merge as nathanjohnpayne after step 6 is clean. Done.
8. If the PR meets the threshold, proceed to Phase 4 (see REVIEW_POLICY.md § Phase 4). Before Phase 4a, consult `phase_4b_default` from `.github/review-policy.yml` (parsed and exported as `PHASE_4B_DEFAULT` by `scripts/codex-review-check.sh`). When set to `always`, post the Phase 4b handoff for every threshold PR after 4a clearance. When set to `complex-changes`, run `scripts/phase-4b-classifier.sh <PR#>` between 4a clearance and merge — exit 1 means post the Phase 4b handoff and wait for an external CLI review; exit 0 means merge after step 6 is clean. Full detail lives in REVIEW_POLICY.md § Phase 4b Triggers. Phase 4 has two legs:
...
10. Merge as nathanjohnpayne only after the merge gate and step 6 are clean. Non-Dependabot auto-merge is opt-in only:
