...
- GitHub does not expose prior bodies of an edited issue comment through the paginated comments API. On a configured bot's PR-comment edit or deletion, the trusted default-branch workflow renders the previous marker set into an append-only `github-actions[bot]` archive record before running the gate. Accounting validates that record, binds it to the configured source bot and source comment id, and retains every distinct archived finding version while the live source is markerless or deleted; duplicate deliveries collapse by source id and body fingerprint. A live re-raise supersedes its archives. Each archive uses the normal source-comment acknowledgement token with a post-archive evidence floor, so a pre-rewrite acknowledgement cannot clear it.
...
- Event-driven Codex P1 publishers serialize per pull request, and both event and scheduled publishers fingerprint all three feedback surfaces before and after evaluation. A changed fingerprint rejects publication, so an older run cannot post a stale success after a newer finding, edit, deletion, or acknowledgement change.
