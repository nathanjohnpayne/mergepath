...
- A `transport` cause means the probe **refused to run**, because the configured SSH transport cannot be made non-interactive and could therefore stall an unattended audit on a prompt. Two shapes reach it: `GIT_SSH` alone, which names a program with no place to inject options; and a `GIT_SSH_COMMAND` / `core.sshCommand` that pins `BatchMode` to a non-yes value, which an appended option cannot override because OpenSSH keeps the first value of a repeated option. A visible incomplete audit is preferred to an invisible stall — the guarantee is kept honest rather than nearly true. The check is textual and deliberately conservative; it cannot see inside a wrapper script that sets `BatchMode` itself, and errs toward refusing.
