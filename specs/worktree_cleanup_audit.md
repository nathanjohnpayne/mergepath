...
Only a failure whose absence of output is indistinguishable from a real answer. Today that is the single remote-heads snapshot behind the stale-unpruned probe, which is dry-run only — `--apply` never exits `3`.

- The probe reports one of three states. `ok`: the snapshot was taken and the reported set is authoritative. `declined`: `remote.origin.fetch` is not the conventional `+refs/heads/*:refs/remotes/origin/*`, so the tracking-ref-to-remote-head mapping is unknowable and the probe does not guess. `unknown`: the read failed and nothing is known either way.
