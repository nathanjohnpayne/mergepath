...
- The probe cannot hang. Its remote read runs under a wall-clock bound that always exists — `timeout` where present, which bounds the whole process tree, and otherwise a shell watchdog that bounds the audit. That bound, not an inspection of the SSH configuration, is what makes an unattended run safe: a mixed-value `BatchMode` that OpenSSH's first-value rule defeats, a wrapper script that sets `BatchMode` itself, an HTTPS credential helper that blocks, and a legacy `GIT_SSH` with nowhere to inject options all reduce to "takes too long" and surface as a `remote` cause. An earlier revision refused to run on a textual reading of the ssh command instead; it could not cover that space, and it wrongly refused HTTPS and local remotes for ssh settings that could not affect them.
...
The probe is non-interactive and time-bounded (#933). It is a read run from scripts and after every merge, and the caller hides stderr, so a prompt would stall an unattended audit invisibly — a hang is a worse outcome than a refusal. Every prompt path is therefore turned into an immediate non-zero exit, which the probe records as `unknown` with a `remote` cause: `GIT_TERMINAL_PROMPT=0` for git's own prompt, `GIT_ASKPASS` / `SSH_ASKPASS` plus `SSH_ASKPASS_REQUIRE=never` for the GUI path, and `ssh -o BatchMode=yes` so a passphrase or an unknown host key fails rather than asking. Stalls are bounded by `ssh -o ConnectTimeout` and by `http.lowSpeedLimit` / `http.lowSpeedTime`.

Three constraints on that hardening are deliberate. `BatchMode` is not paired with `StrictHostKeyChecking=accept-new`, because silently trusting an unknown host key to keep an audit quiet trades a visible stall for an invisible trust decision. Credential helpers are untouched, because the goal is to block prompts rather than cached credentials, and disabling them would break the audit on every private remote where it works today. The ssh options are appended to any operator `GIT_SSH_COMMAND`, and ssh honours the first occurrence of a repeated `-o`, so an operator who set one of these explicitly keeps their value.

The wall-clock ceiling applies only where a `timeout` implementation exists. `timeout(1)` is GNU coreutils and a stock macOS has neither it nor `gtimeout`, so requiring it would disable the probe on this audit's primary host; the environment and ssh/http bounds are the portable floor and already close every prompt path. `WORKTREE_CLEANUP_PROBE_TIMEOUT` moves the ceiling, and the connect timeout is derived from it so one knob moves both.
