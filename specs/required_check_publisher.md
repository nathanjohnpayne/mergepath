...
The contract stated below is larger than what `scripts/ci/check_required_check_publisher` currently enforces, and the boundary is deliberate rather than incidental. **Every property the fence pins is decided over the parsed workflow document**, and Test 25 in `tests/test_merge_clearance_gate.sh` proves each pin rejects the shape that loses it — every failure the fence can emit has a mutation case that trips it, so no assertion here passes vacuously. That is the whole of the enforcement claim; a spec sentence is not evidence that anything checks it.
