...
Blockquote nesting is deliberately left outside the ceiling. It reaches the same depths for the same two bytes per level, but it does not reach the same cost: measured on this runtime it takes 96ms at 5,000 levels, 191ms at 15,000, 1,042ms at 30,000 and 1,256ms at 32,760, the deepest a 65,536-character body can express. That is superlinear but already bounded by the body-size limit at about 1.3 seconds, where list nesting at the same depth exceeded 45 seconds. Extending the ceiling to blockquotes would widen a semantic cutoff for no cost benefit, so the regression control pins the measurement instead.
...
`tests/test_pr_body_contract_parity.sh` verifies the stable CLI result and all existing consumers' shared-parser use. Its #1192 corpus covers renderer-confirmed quote-first-block, list-transition, nested-list, comment, code, and malformed-heading outcomes. Its #1281 controls also pin footnote-definition membership, including the negative cases where a blank line or an interrupting heading legitimately restores top level, and the blockquote-depth measurement above. They pin the narrow guarantee the ceiling actually makes: the pathological body parses in bounded time, and a differential against an unbounded build of the same bundle observes no divergence. That differential asserts its own measured coverage first---how many generated lines crossed the ceiling, and whether every marker and content shape was drawn---because an earlier version used a generator whose float multiplication collapsed the depths to a set that never reached the ceiling, and so reported zero divergence against a ceiling it had not exercised.
