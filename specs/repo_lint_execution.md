...
An approval records merge readiness in GitHub's review state; it does not reserve a runner while required checks execute. The approval workflow performs a one-shot current-head check probe and exits successfully when work is still pending. The trusted default-branch continuation listens for approval, canonical repo-lint, optional local-annex, and policy-gate completion events. Because `workflow_run` accepts only literal workflow display names while a consumer annex may use any name, the existing five-minute scheduled sweep also re-enters the continuation for open approved pull requests. Each entry resolves the live open pull request by number or exact head SHA, revalidates a latest-state registered reviewer approval plus current-head required and optional-annex checks, then runs the threshold-aware merge-clearance predicate. Immediately before arming native auto-merge it re-reads the current head, all blocking labels, feedback accounting, and unresolved conversations. It passes the re-read head to `--match-head-commit`. A pending or failed prerequisite is a no-op for that event and is reconsidered by a later completed-workflow event or sweep.
