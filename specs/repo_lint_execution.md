...
An approval records merge readiness in GitHub's review state; it does not reserve a runner while required checks execute. The approval workflow performs a one-shot current-head check probe and exits successfully when work is still pending. Both the immediate-green approval path and every later re-entry use the same trusted default-branch continuation. That continuation listens for approval, canonical repo-lint, optional local-annex, and policy-gate completion events. Because `workflow_run` accepts only literal workflow display names while a consumer annex may use any name, the existing five-minute scheduled sweep also re-enters the continuation for open approved pull requests. Each entry resolves the live open pull request by number or exact head SHA, revalidates a latest-state registered reviewer approval plus current-head required and optional-annex checks, then runs the threshold-aware merge-clearance predicate. It then requires complete feedback accounting and zero unresolved conversations. Immediately before an eligible automated arm it re-reads the pull-request state, draft status, current head, base ref/SHA, and all blocking labels; re-resolves approval independence from complete live metadata and review history; and passes the re-read head to `--match-head-commit`. A pending or failed prerequisite is a no-op for that event and is reconsidered by a later completed-workflow event or sweep. Shared-author Phase 4 PRs are the exception: their authoring identity lives in mutable PR metadata that GitHub's merge precondition cannot bind, so the continuation does not leave auto-merge armed for that class and retracts an existing request when found. The ordinary one-shot author merge remains available after the live gates are clean. Under-threshold shared-author PRs and native non-shared Phase 4 authors retain the automated path.
