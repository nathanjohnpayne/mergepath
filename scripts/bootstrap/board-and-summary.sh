...
"
      ...
      github-infra)
        if [ "$in_state" = "1" ]; then
          done_lines="$done_lines  [done] GitHub repo created ($visibility) + 19 labels + reviewer invites + secrets
"
        else
...
"
...

  {
    ...
    echo "  NOTE: this loop-doc PR does NOT enroll '$repo_name' for ongoing"
    echo "  sync. The one-time template rsync above and .mergepath-sync.yml"
    echo "  consumer enrollment are SEPARATE steps — a repo registered only"
    echo "  in the loop docs receives none of mergepath's post-bootstrap"
    echo "  propagation. Enroll it as a consumer per NEXT STEPS below so the"
    echo "  weekly propagation-drift sweep tracks it (see mergepath#741)."
    echo
    ...
    echo "  2. Enroll '$repo_name' as a .mergepath-sync.yml consumer so it"
    echo "     receives ongoing propagation (not just the one-time rsync):"
    echo "     add it to the consumers: list with a facts: block verified"
    echo "     against its own package.json (frameworks / testing / jsx_in_js"
    echo "     / ESM-vs-CJS eslint variant), then run"
    echo "     scripts/sync-to-downstream.sh --audit --repos $repo_name and"
    echo "     drive the first sync canary-first. See mergepath#741 and the"
    echo "     .mergepath-sync.yml header."
    echo "  3. Write the repo-local implementation spec:"
