...
1. `rsync` the mergepath worktree into the target dir, honoring the exclude list (mergepath-only files, packaging dirs, screenshots, and the pure-identity `BRAND.md` / `docs/agents/repository-overview.md` and the hub glossary `CONTEXT.md` — see step 5) plus the hub-only machinery docs, whose exclude patterns are derived from the `doc_ownership:` inventory at run time rather than listed in the array. If that derivation fails for any reason, the rsync aborts instead of mirroring without the hub-only exclusions.
...
7. Initialize the new repo's git history with a single initial commit. When `source_root` is eligible (its origin names canonical mergepath exactly, `git status --porcelain` there is clean, and `HEAD` is reachable from `source_root`'s own `refs/remotes/origin/*`), the commit carries both the subject naming which mergepath revision the bootstrap was based on — `"Initial commit (bootstrapped from mergepath@<sha7>)"` — and a `Source: https://github.com/nathanjohnpayne/mergepath/commit/<sha>` trailer ([#1056](https://github.com/nathanjohnpayne/mergepath/issues/1056)); this is the recoverable starting `HUB_REF` that [`docs/agents/propagation-ordering.md`](propagation-ordering.md) § Measuring tier membership uses. Informational provenance, not proof the resulting tree is byte-for-byte derivable from that sha — see [`specs/bootstrap_source_attribution.md`](../../specs/bootstrap_source_attribution.md). Best-effort: an ineligible source_root — including an operator's own not-yet-pushed local commit, which is not a sha anyone else can recover — falls back to the un-attributed `"Initial commit (bootstrapped from mergepath)"` subject instead of blocking the bootstrap.
...
1. `gh repo create --source=.` against the target dir, then a separate `git push -u origin HEAD` — creates the remote, then pushes the bootstrap commit. Legitimate push to main on a greenfield remote (no `main` to protect yet). The two are deliberately separate commands: bundled as `gh repo create ... --push` they shared a single exit code, so a push that failed on a transient network or credential error — with the repository already created — reported failure before any checkpoint was written, leaving a remote that nothing recorded (#790). Split, the create's success is checkpointed immediately and the push is an ordinary repeatable step a resume retries. Splitting the commands does **not** split them off the author credential: the push runs through `bootstrap::run_author_git`, which routes it through the same `scripts/gh-as-author.sh` wrapper as the create and binds git's credential to the token that wrapper verified (`-c credential.helper=` to clear inherited helpers, then `!gh auth git-credential` to supply the wrapper's `GH_TOKEN`), with `GIT_TERMINAL_PROMPT=0` and an empty `GIT_ASKPASS` so a credential miss fails instead of prompting. That is the same wiring `gh` used internally for its own `--push`. A bare `git push` here would take the machine's ambient credential-helper state instead — failing or prompting on a clean setup, and populating the new repo's `main` under the wrong identity wherever another configured account has access.
2. Seed the 19 canonical labels (`needs-external-review`, `needs-human-review`, `policy-violation`, `human-hold`, `human-action`, `decision-needed`, `agent-action`, `phase-0` through `phase-4`, `size:S`/`size:M`/`size:L`, and `priority:critical`/`priority:high`/`priority:normal`/`priority:low`). The `size:` and `priority:` values are the fleet-shared subset agreed in the 2026-09-04 backlog audit and are byte-identical to mergepath's own; `area:*`, `status:*` and repo-local `type:*` values are deliberately per-repo and are not seeded.
...
The wizard does not provision `AUTHOR_MERGE_TOKEN` by default, but `dependabot-auto-merge.yml` **requires it** as of nathanjohnpayne/mergepath#426: the trusted `pull_request_target` workflow uses it for the `gh pr merge` step so the merge is recorded under `author_identity`. Before enabling that lane or #1058, the owner manually creates the protected `merge-queue-policy` environment with administrator bypass disabled, no reviewers or wait timer, and exactly one custom branch named `main`. `AUTHOR_MERGE_TOKEN`, target-only `MERGE_QUEUE_POLICY_TOKEN`, and public-source-only `MERGE_QUEUE_SOURCE_TOKEN` must be pairwise distinct and stored only in that environment, never at repository or organization scope; candidate-controlled workflows reference none. For #1058 the observer uses the author token only for its exact-head marker and exact entry attestation, while the protected target `repository_dispatch` final auditor uses it only for the nonce-bound final marker; those are the only #1058 author-token writers. The target policy token performs only read operations through REST `GET` requests and fixed GraphQL queries, with organization and target `Administration: write` for bypass evidence plus target `Actions: read`, `Secrets: read`, `Environments: read`, `Contents: read`, `Issues: read`, `Pull requests: read`, and automatic metadata; the source token performs read-only public source-workflow reads with source `Actions: read` and automatic metadata. Both read tokens are runtime requirements for every live `enqueued` observer attestation and every final administrative audit, not credentials used only during manual rollout. The organization required workflow's admission, precheck, and postcheck jobs use read-only `github.token`; its one intervening exact-SHA source-pinned handshake has job-scoped `contents: write` solely for one same-repository dispatch, never for content, PR, queue, protection, or merge mutation. None of those paths grants Agent Review authority to arm, enqueue, dequeue, directly merge, disable non-Dependabot native auto-merge, or alter queue/protection settings; see [`../../specs/merge_group_required_checks.md`](../../specs/merge_group_required_checks.md).
...
Because that record is what makes the remaining failures recoverable, a checkpoint write that does not land is itself a stage failure. Step 1 checks `bootstrap::record_checkpoint` and aborts before the push if the sidecar could not be appended (unwritable path, full disk), naming the exact line to add by hand. Stage functions run under `|| stage_rc=$?`, so `errexit` is disabled through the call chain and an unchecked write would have carried on into the push and the label / invite / secret steps — any of which could then abort into precisely the unrecoverable state the checkpoint exists to prevent: remote created, nothing recording it, create not repeatable, `--resume` refused by preflight (#790).
...
10. **Activate the native merge queue separately, if desired.** The wizard leaves rollout `disabled` and configures no queue, environment, secret, source, or external rule. Follow [`../../specs/merge_group_required_checks.md`](../../specs/merge_group_required_checks.md): manually merge the reviewed implementation while disabled; verify organization entitlement; transfer the target; create and protect an organization-owned **public** policy repository; materialize, exact-byte audit, individually disable, and read back both source workflows; exact-byte audit the source-pinned handshake and target `main` final-auditor workflow; and retain zero target `merge_group` triggers. The source must be public for every target visibility because the handshake fetches its exact SHA without credentials. Replace generic bootstrap branch protection with the canonical exact six Actions-owned contexts, strict freshness, administrator enforcement, stale-review dismissal, no pull-request-review bypass, and conversation resolution. Create both no-bypass rulesets in `evaluate`. Create `merge-queue-policy` with administrator bypass disabled, no reviewers or wait timer, and exactly one custom branch named `main`. Store pairwise-distinct `AUTHOR_MERGE_TOKEN`, target-only `MERGE_QUEUE_POLICY_TOKEN`, and public-source-only `MERGE_QUEUE_SOURCE_TOKEN` values only in that environment; candidate-controlled workflows reference none. The policy token needs organization and target `Administration: write` for bypass evidence plus target `Actions: read`, `Secrets: read`, `Environments: read`, `Contents: read`, `Issues: read`, `Pull requests: read`, and automatic metadata, but performs only read-only REST `GET` requests and fixed GraphQL queries; the source token needs only source `Actions: read` and automatic metadata. Both read tokens remain available to every trusted live `enqueued` observer and protected final audit because each reruns the target and source proofs. Before activation and after promotion, verify both read-token identities, the exact environment/custom-branch policy, complete repository/inherited-organization/environment secret inventories, exact branch protection, both exact rulesets, both disabled source workflows, and the exact target final-auditor runtime guards before environment access. Activate the organization workflow ruleset first and the repository queue ruleset second; rollback removes them in reverse order. Admission neutrally passes drafts but rejects SHA-like head names, stacks, and target-workflow changes; enrollment and both read-only group checks still reject drafts and stacks. The trusted observer handles `auto_merge_enabled` and `enqueued`, writing the exact-head marker and exact post-enqueue entry attestation after live checks. The read-only precheck bounded-polls for the attestation; the one job-scoped `contents: write` handshake executes exact-SHA public source code, sends one nonce-bound same-repository dispatch, and never retries ambiguity; the target auditor applies exact actor/repository/action/`main`/base/workflow guards before secrets, rechecks private topology/action/queue state, and writes one unique author marker; the handshake requires that marker plus the exact successful time-bound auditor run; and read-only postcheck replays every public predicate plus the final binding. The human-only canary must additionally prove the dispatch run has both `actor.login` and `triggering_actor.login` exactly `github-actions[bot]`. From handoff through native merge, freeze every mutable administrative, policy-source, rollout, PR/review/comment/thread/accounting, check/status, queue/action, default-base, and group input; any mutation ejects or aborts the entry and requires a fresh owner action and group. The canary must still prove cleared arm fields, `human-hold` ejection, and no automatic re-enrollment. Only the human owner arms, enqueues, merges, or removes `human-hold`. The wizard and #1058 automation perform none of the rollout actions.
...
- `merge-queue-policy` and its three secrets unset by default. The wizard does not create the environment or provision any of `AUTHOR_MERGE_TOKEN`, `MERGE_QUEUE_POLICY_TOKEN`, or `MERGE_QUEUE_SOURCE_TOKEN`.
- `AUTHOR_MERGE_TOKEN` required wherever Dependabot auto-merge is enabled (#426) and for #1058's trusted observer and protected final auditor. It is pairwise distinct from the two read tokens, exists only in the protected environment, and writes only the observer marker/attestation or nonce-bound final-audit marker.
- `MERGE_QUEUE_POLICY_TOKEN` required before #1058 activation, for every live `enqueued` attestation, and for every final audit as a target-only fine-grained PAT with organization and target `Administration: write` plus target `Actions: read`, `Secrets: read`, `Environments: read`, `Contents: read`, `Issues: read`, `Pull requests: read`, and automatic metadata. It exists only in the protected environment and performs only read-only REST `GET` requests and fixed GraphQL queries.
- `MERGE_QUEUE_SOURCE_TOKEN` required before #1058 activation, for every live `enqueued` attestation, and for every final audit as a public-policy-source-only fine-grained PAT with `Actions: read` and automatic metadata. It exists only in the protected environment and performs only read operations.
