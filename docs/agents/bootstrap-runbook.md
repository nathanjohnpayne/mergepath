...
2. **Enroll the repo as a `.mergepath-sync.yml` consumer.** The wizard does the one-time template rsync and the loop-doc registration, but NOT ongoing-sync enrollment — a repo left unenrolled receives none of mergepath's post-bootstrap propagation and never appears in the weekly `propagation-drift` sweep (the mergepath#741 gap). Add it to `consumers:` with a `facts:` block verified against its own `package.json` (`frameworks` / `testing` / `jsx_in_js` / ESM-vs-CJS eslint variant), run `scripts/sync-to-downstream.sh --audit --repos <repo>`, and drive the first sync canary-first — a newly enrolled repo is the natural canary for the wave that reconciles its own backlog, which is a per-wave selection reason in [propagation-ordering.md](propagation-ordering.md) and **not** a hold on the rest of the fleet; fan-out is gated on the canary going green plus the wave audit, never on any repo's tier membership. Before that first sync, also confirm whether the repo needs any `.repo-template.yml` or per-consumer `exclusions:` adjustments (mirroring the ffb/dpr precedent) and record the result, rather than discovering gaps mid-PR.
