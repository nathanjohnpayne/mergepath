...
7. Initialize the new repo's git history with a single `"Initial commit (bootstrapped from mergepath@<sha7>)"` commit, plus a `Source: https://github.com/nathanjohnpayne/mergepath/commit/<sha>` trailer naming the mergepath tree it was mirrored from ([#1056](https://github.com/nathanjohnpayne/mergepath/issues/1056)) — the recoverable `HUB_REF` that [`docs/agents/propagation-ordering.md`](propagation-ordering.md) § Measuring tier membership needs. This requires the source root to pass every canonical-source check in [`specs/bootstrap_source_attribution.md`](../../specs/bootstrap_source_attribution.md) (exact git top level, an origin naming canonical mergepath, HEAD in that repo's own remote history, and a clean working tree). Best-effort: failing any one of those falls back to the un-attributed `"Initial commit (bootstrapped from mergepath)"` subject instead of blocking the bootstrap.
