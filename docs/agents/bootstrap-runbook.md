...
7. Initialize the new repo's git history with a single initial commit. When `source_root` is eligible (its origin names canonical mergepath exactly, and `git status --porcelain` there is clean), the subject names which mergepath revision the bootstrap was based on — `"Initial commit (bootstrapped from mergepath@<sha7>)"` — with a `Source: https://github.com/nathanjohnpayne/mergepath/commit/<sha>` trailer added only when that revision is reachable from `source_root`'s own `refs/remotes/origin/*` ([#1056](https://github.com/nathanjohnpayne/mergepath/issues/1056)); this is the recoverable starting `HUB_REF` that [`docs/agents/propagation-ordering.md`](propagation-ordering.md) § Measuring tier membership uses. Informational provenance, not proof the resulting tree is byte-for-byte derivable from that sha — see [`specs/bootstrap_source_attribution.md`](../../specs/bootstrap_source_attribution.md). Best-effort: an ineligible source_root falls back to the un-attributed `"Initial commit (bootstrapped from mergepath)"` subject instead of blocking the bootstrap.
