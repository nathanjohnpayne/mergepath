...
MCP Server only for attended Codex Environment workflows, use the
1Password local `.env` validation hook for supported non-Codex agents
that read mounted Environment files, and use `.env.tpl` + `op inject`
only when the repo truly needs a generated config file on disk. Adoption
decisions for these adapters belong to the 1Password audit ADR
