...
The carve-out bullet above is a verbatim copy of the shared core's; it is stated here only so this overlay does not read as a contradiction on its own. The fleet-wide statement lives in [`shared-operating-rules.md`](shared-operating-rules.md), which is the copy every consumer receives — edit it there.
