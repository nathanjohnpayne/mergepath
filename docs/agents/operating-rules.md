...
- **The single exception is the pivot marker**, and it overrides the two bullets above. When a PR reverses direction — the change-level trigger defined in `docs/agents/decision-records.md`, which explicitly excludes ordinary rounds of review fixes — its title carries a trailing `(pivoted from <one-phrase abandoned approach>)`. It earns the exception because a squash merge makes the PR title the commit subject, so the marker is the only pivot signal that reaches `git log`, where a reader tracing a file's history actually looks. It is a fixed, greppable marker pointing at the `## Path taken` record, not prose narration — and it does not relax the rest of the title, which still describes the end state.
