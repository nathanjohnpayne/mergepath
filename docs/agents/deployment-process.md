...
- The 1Password-first deploy-auth model is a deliberate default here, and the no-unilateral-downgrade rule above is what forbids swapping it for routine browser-login or `firebase login` when a lookup fails.
...
- The Firebase-vault SA key in 1Password is this repo's supported on-account form of the "no long-lived keys by convenience" rule; on-disk deploy keys are not an accepted substitute for it.
- If credential preflight was run at session start with deploy creds loaded (`scripts/op-preflight.sh --mode deploy` or `--mode all`), deploy credentials are usually already cached, per Firebase-project context. In a Firebase repo with a `.firebaserc` default project, deploy preflight caches the project Firebase-vault SA key first and only falls back to the shared GCP ADC when that key is absent. No additional biometric prompt is needed for deployment. **`--mode all` can succeed without deploy credentials:** when neither the SA key nor a usable ADC loads, it still exports the PATs and sets `OP_PREFLIGHT_DONE=1`, warns on stderr, and reuses that degraded result for `OP_PREFLIGHT_DEPLOY_DEGRADED_BACKOFF_SECONDS` (default 900s). Before deploying after `--mode all`, confirm `GOOGLE_APPLICATION_CREDENTIALS` is set; if it is not, run `scripts/op-preflight.sh --mode deploy` (which fails closed with the reason) or fix the credential and use `--refresh`, rather than letting the deploy fall through to another auth source. The default `--mode review` does NOT load deploy credentials (#282); pass `--mode deploy` explicitly for a deploy session.
