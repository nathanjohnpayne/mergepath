...
| **3** | `device-source-of-truth` + `friends-and-family-billing` | Both confirmed 2026-09-01. Recently touched by the ESLint-floor work. **`device-source-of-truth` became private during this window** (verified 2026-09-01: anonymous API access returns 404), making it the fleet's only non-public consumer and the one repo whose CI failures need auth to read; its manifest entry still records `visibility: public`. It also ranks third on churn (21 locally-authored PRs) and took consumer-local CI failures on a sync PR (its #167: `Contract Gate`, `Frontend (Smoke + Coercion)`), superseded by a later merged re-cut. Held at wave 3 rather than promoted: its config divergence is nil (no overrides, plain `react, typescript` facts), and the visibility flip bears on canary readability rather than on fan-out clobber risk. Re-examine next month if the churn rank persists. friends-and-family-billing: divergence still entirely template-folded (`testing: vitest`, `jsx_in_js: true`), no bespoke override. |
