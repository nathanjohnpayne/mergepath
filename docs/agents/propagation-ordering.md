...
Each partial invocation forces the Phase 4b orchestrator's dry-run path and retains a `wave-audit-prefix/<full-head>/<prefix-end>` receipt only when the complete schema-validated verdict is `APPROVED` with `findings: []`. An APPROVED verdict carrying advisories is a findings outcome here: the command returns the complete verdict for disposition and retains nothing. Successful partial progress exits 9 and its JSON states `clearance: false`, `watermark_advanced: false`, and `fanout_authorized: false`; callers must never interpret it as ordinary approval. A top-level `--dry-run` validates the chunk but writes neither a prefix receipt nor the full watermark.
