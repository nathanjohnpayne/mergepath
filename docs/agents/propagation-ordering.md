...
**Whether there is a hub sha to recover here depends on when the consumer was bootstrapped.** Before [#1056](https://github.com/nathanjohnpayne/mergepath/issues/1056), `scripts/bootstrap/template-mirror.sh` committed the mirrored tree as `Initial commit (bootstrapped from mergepath)` and recorded no source sha anywhere — not in that message, not in the mergepath-side loop PR body, and nowhere in the logged commands (verified against `gaycruisebingo`'s `a2965a93`). Sending an operator to look for it wastes the lookup and, worse, invites reaching for a *plausible* mergepath commit by date, which silently produces a baseline the mirror never used. **This is still true for every consumer bootstrapped before #1056 landed** — the fix is not retroactive, and no amount of digging recovers a sha the mirror never wrote down.
...
| `tadlockpsychiatry` | `e813d23381ee71e18fa442d2f4bb29206e964659` | | `gaycruisebingo` | `767341cc996382e4914225a89467f94d34409eca` |
