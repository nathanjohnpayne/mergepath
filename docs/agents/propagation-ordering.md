...
## The pr-review-policy recovery lane travels as a closure (#931)

`.github/workflows/pr-review-policy-recovery.yml` is a **canonical, consumers-all** entry that shells out to `scripts/pr-review-policy-recovery.sh`, which in turn hard-sources five libraries and invokes `scripts/merge-clearance-gate.sh`. It publishes two **required** contexts on every consumer, so a partial delivery is not cosmetic. Wave implications:

- **The workflow must never lead its script.** Same hazard shape as the `validate-pr-body.sh` ordering above, and with the same consequence class: a consumer that receives the workflow without the script runs a `*/15` sweep that exits 127 every quarter hour and recovers nothing, silently. The manifest closes this structurally — the workflow entry's `requires:` names the script and its complete runtime closure, and the commit-path closure gate reads each selected target's direct requires — so a path-scoped wave cannot split them. Do not hand-order around it; if a wave looks like it needs to, the manifest edge is what to fix.
- **The gate workflow and the lane are a lockstep pair.** `scripts/ci/check_pr_review_policy_recovery` asserts that the two context names the lane publishes are exactly the `name:` values of `pr-review-policy.yml`'s `self-review-check` and `label-gate` jobs. A wave that renames either job without the lane reds `lint` on every consumer it reaches.
- **A consumer with a large open-PR population needs the budget checked first.** The sweep's steady state is roughly `8 × open_PR_count` requests per hour against a `GITHUB_TOKEN` allowance of 1,000 per hour per repository. See `specs/pr_review_policy_recovery.md` § Budget for the arithmetic and the ceiling.
