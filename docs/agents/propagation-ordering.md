...
Prefix receipts bind the initial base, intended full head, intended-head manifest blob, configured scope fingerprint, exact contiguous chunk range, curated-diff object, and complete clean verdict. The intended-head manifest scope stays fixed across all chunks. When a path enters that scope during a chunk, the chunk covers its full content at the admission commit, including bytes that predate the manifest change. Receipt drift, ambiguity, disconnection, or an incomplete prefix fails closed.

`--finalize-historical` is the only operation that can turn those receipts into clearance. It submits a labeled cumulative-coverage receipt package through the existing non-dry Phase 4b publication and accounting path; the package explicitly identifies itself as receipts rather than a code diff or prior posted approval. Only a successful final verdict advances `wave-audit-pass/<full-head>` and permits fan-out. Any finalization failure leaves that watermark unchanged. A partial command never starts finalization implicitly, even when its end equals the intended head.
