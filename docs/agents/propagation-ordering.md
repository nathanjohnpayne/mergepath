...
**The dominant failure mode for this change is uniform, and it is already evidenced.** The immediately preceding wave (hub `0e203d0`) failed on all eight consumers for one reason: the parity suite's Phase 4b probes assumed the automation was enabled, and no consumer carries a `phase_4b` block, so `phase-4b-review.sh` exited 5 on the disabled path and the assertion misread that as a contract failure. Green on the hub, red everywhere else. That is the #264 class exactly — a payload that exercises a path the hub has and consumers do not — and it reproduced identically on every repo rather than on the divergent ones. mergepath#1153 fixed it by driving the probes through the documented `--force-enabled` override; this wave is the first to carry that fix outward, which is the specific thing the canary is being asked to prove.
