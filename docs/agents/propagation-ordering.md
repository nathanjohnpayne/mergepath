...
|---|---|---|
| **0 (first-sync reason consumed — held on conservatism)** | `fiveacross` (single) | **The first-sync signal is spent, recorded 2026-08-30.** The 2026-08-20 measurement placed it here because its `.sync-overrides.yml` had skipped the **entire** `scripts/ci/` kit until fiveacross#1011 removed the skip at 2026-08-20T00:33Z, making the next wave its first delivery of that whole directory. That delivery has now happened: the corrective wave at hub `0e203d0` merged as fiveacross#1040 on 2026-08-29, so it holds the kit like every other consumer and no future wave is its first sync of anything. The clobber-sensitive file that reason carried—`check_workflow_parsers`, holding 71 lines of local `synthetic-uptime.yml` assertions (#768)—was consumed by that same delivery. **What this row is now:** a conservative default, not an evidenced tier. No current measurement ranks `fiveacross` above the tier-1 repos, and the config-divergence axis has never been measured for any consumer; it stays first only because leading with the most-recently-reconciled large surface is the cheaper mistake. Re-measure before relying on this position ([§ Measuring tier membership](#measuring-tier-membership)); if the next measurement does not distinguish it, fold it into tier 1. **Distinguish this from the earlier error:** an earlier round read the 2026-08-20 *measurement* as retiring the tier, and that reading was wrong—the measurement re-based the same position on a signal that does reproduce. This retirement rests on a different and newer fact, that the first sync has actually occurred. |
