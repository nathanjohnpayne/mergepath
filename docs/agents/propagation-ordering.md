...
The result JSON's `base_watermark` object and a stderr line report the selected tag, its annotation's `tagger_timestamp` (Unix seconds), and elapsed `age_seconds`, including for over-budget and reviewer-unavailable outcomes. This is unsigned tag metadata, not the commit date, last review time, or verified review freshness. Explicit `--base` selects no watermark tag and reports null fields. Missing, unreadable or invalid annotation timestamps produce an unknown age (`null`); a future timestamp remains visible but its age is unknown. Age does not change selection, dispatch, exit codes or watermark advancement and has no staleness threshold.

An over-budget historical range can be reviewed as an operator-chosen contiguous sequence without weakening that contract. Pin the original base and intended full head on every invocation, advance only to the next chosen commit, and finalize separately after the retained prefix reaches the intended head:

```bash
scripts/wave-audit.sh <canary-pr> --repo <owner>/<canary-repo> --base <initial-base> --head-sha <full-head> --historical-end <next-end>
# Repeat the same command with the next contiguous end. Then, explicitly:
scripts/wave-audit.sh <canary-pr> --repo <owner>/<canary-repo> --base <initial-base> --head-sha <full-head> --finalize-historical
```

Each partial invocation forces the Phase 4b orchestrator's dry-run path and retains a `wave-audit-prefix/<full-head>/<prefix-end>` receipt only when the complete schema-validated verdict is `APPROVED` with `findings: []`. An APPROVED verdict carrying advisories is a findings outcome here: the command returns the complete verdict for disposition, retains nothing, and exits 1. That exit 1 is a historical findings outcome rather than the posted `CHANGES_REQUESTED` of the ordinary path below; disposition the findings instead of re-cutting the wave. Successful partial progress exits 9 and its JSON states `clearance: false`, `watermark_advanced: false`, and `fanout_authorized: false`; callers must never interpret it as ordinary approval. A top-level `--dry-run` validates the chunk but writes neither a prefix receipt nor the full watermark.

Prefix receipts bind the initial base, intended full head, intended-head manifest blob, configured scope fingerprint, exact contiguous chunk range, curated-diff object, actual reviewer direction/identity/adapter/canary-head provenance, and complete clean verdict. The intended-head manifest scope stays fixed across all chunks. When a path enters that scope during a chunk, the chunk covers its full content at the admission commit, including bytes that predate the manifest change. A path present at both the initial base and intended head keeps its ordinary deltas even if an intermediate manifest removes and later re-admits it, so removed old lines remain covered. A boundary manifest that is genuinely absent contributes empty scope; an existing manifest that cannot be read fails before review or retention. Receipt drift, ambiguity, a non-forward range, disconnection, or an incomplete prefix fails closed.

A historical chunk with zero in-scope bytes exits 3 before dispatch and retains nothing, because the Phase 4b adapters require non-empty review input and a prefix receipt requires actual clean reviewer evidence. Choose a later endpoint that coalesces the empty interval with a non-empty chunk; the command never manufactures vacuous reviewer authority.

If a clean receipt is created locally but its push fails, rerun the identical partial command. It publishes that exact validated receipt without spending another review and still returns partial-progress exit 9. A later checkout fetches the remote receipt and continues from its end.

`--finalize-historical` is the only operation that can turn those receipts into clearance. It submits a labeled cumulative-coverage receipt package through the existing non-dry Phase 4b publication and accounting path; the package explicitly identifies itself as receipts rather than a code diff or prior posted approval. Only a successful final verdict advances `wave-audit-pass/<full-head>` and permits fan-out. Every nonzero finalization result leaves that watermark unchanged and denies fan-out, including reviewer-unavailable exits 4/5 that ordinary non-historical audits may treat as fail-open. A partial command never starts finalization implicitly, even when its end equals the intended head.
...
- **Over-budget (exit 8):** the complete curated diff exceeds `propagation_audit.diff_max_bytes`. The helper reports the measured bytes and budget before dispatching the review orchestrator, including in `--dry-run`; no provider wait, review, or watermark advancement occurs. Do not fan out on this result or treat it as transient reviewer unavailability: retrying the same or a larger range cannot fit. Use the explicit historical-prefix procedure above; the refusal never splits the range, raises the budget, or omits content itself. A range exactly at the budget remains eligible for the normal review path.
- **Historical partial progress (exit 9):** one clean chunk was validated and, unless the top-level command used `--dry-run`, retained as a non-clearance prefix receipt. Do not fan out. Continue the pinned sequence or run the separate explicit finalization once it is complete.
...
**Resolving the last synced hub sha** (the `HUB_REF` for a "has this consumer drifted since its last wave?" comparison): `scripts/sync-to-downstream.sh --sync-all` records it in three places, and only one of them is abbreviated. The **branch name** `mergepath-sync/sync-all-<sha7>` carries 7 characters. The **sync commit message** carries a `Source: https://github.com/nathanjohnpayne/mergepath/commit/<sha>` trailer with the full sha. The **PR body's first line** reads `Bulk sync to [mergepath@<sha7>](…/commit/<sha>)`, whose link target is also the full sha. Prefer either full-sha form over the branch name. Note the `Source:` trailer is on the COMMIT, not in the PR body — grepping the body for it returns nothing (measured across 92 sync PRs on three consumers). Read it off that consumer's most recent merged sync PR rather than assuming the fleet shares one; they usually do. A consumer that reached parity through the bootstrap template mirror has **no** sync PR at all, so this three-place convention doesn't apply to it.

**Since [#1056](https://github.com/nathanjohnpayne/mergepath/issues/1056), a bootstrapped consumer's own initial commit can carry a starting `HUB_REF`.** `scripts/bootstrap/template-mirror.sh` names the mergepath revision the bootstrap was based on in the commit subject (`Initial commit (bootstrapped from mergepath@<sha7>)`) with a `Source:` trailer carrying the full sha — written together, never one without the other, and only when that revision is reachable from the source checkout's own `refs/remotes/origin/*` (an operator's own not-yet-pushed commit isn't eligible at all, since nobody else could resolve it later) — see `specs/bootstrap_source_attribution.md` for the complete eligibility contract. Treat it as a starting point, not a synced-and-verified baseline the way a sync PR's trailer is: it's best-effort provenance recorded once, at bootstrap time, from whatever the operator's local checkout looked like then.

**A consumer bootstrapped before #1056, or from an ineligible source checkout, still has no recoverable sha.** `scripts/bootstrap/template-mirror.sh` used to commit the mirrored tree as the un-attributed `Initial commit (bootstrapped from mergepath)` with no source sha anywhere — not in that message, not in the mergepath-side loop PR body, and nowhere in the logged commands (verified against `fiveacross`'s `a2965a93`, bootstrapped before this feature existed). Sending an operator to look for one on such a repo wastes the lookup and, worse, invites reaching for a *plausible* mergepath commit by date, which silently produces a baseline the mirror never used. For that repo: **drop the hub baseline and use the consumer-to-consumer comparison below**, which needs no hub ref and answers the divergence question directly.
