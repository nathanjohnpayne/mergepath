...
- Consumer-safe CI gates (#467): `check_workflow_verify_propagation_templated` and `check_verify_propagation_pr` propagate to consumers via the `scripts/ci/` kit, but a wrapped test can be legitimately absent in a consumer checkout. Both wrappers use `.mergepath-sync.yml` as the canonical/consumer discriminator (same shape as `check_bootstrap_sh`): manifest **present** (canonical) → a missing test is a hard error so an accidental delete/rename surfaces in CI; manifest **absent** (consumer) → SKIP rather than red the consumer's repo-lint for a file it never receives.
