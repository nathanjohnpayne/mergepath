...
> **Interaction with `feedback_policy` (#574).** The `feedback_policy` block in `.github/review-policy.yml` can mark the `nitpick` tier `required` (or use `mode: address-all`). That requirement only has teeth under `profile: assertive` — under `chill` the 🧹 Nitpick category is suppressed, so no nitpick threads exist and `nitpick: required` is a **silent no-op**. A repo that genuinely wants nitpicks gated must set `assertive` here as well. (Planned, not yet shipped: when the #577 CodeRabbit severity gate lands it will emit a warning on `nitpick: required` under the `chill` profile. #576 ships the schema + docs only — no gate yet.)
...
So the three **private** consumers share learnings org-wide: a review convention CodeRabbit learns in one private repo can be applied in another repo under the same owner. For a **single-owner fleet** (`nathanjohnpayne`) where cross-repo review conventions *should* carry over, that is the intended behavior, not a leak — the learnings stay within one account's org boundary and CodeRabbit does not use code for model training (FAQ § Data Security).

This is recorded as a **deliberate divergence-awareness**, not a change. A repo that wants its learnings firewalled (a more sensitive private repo) should override to `scope: local` in its own `.coderabbit.yml`. The template stays on `auto` because the template repo is public (so it resolves to `local` here anyway) and because we do not want to impose `local` on every consumer that benefits from shared conventions.
...
2. Verify `nathanjohnpayne` holds an **active** seat and that the seat's repo/org coverage includes all 8 consumers: `matchline`, `overridebroadway`, `tadlockpsychiatry` (private) and `device-source-of-truth`, `friends-and-family-billing`, `device-platform-reporting`, `swipewatch`, `nathanpaynedotcom` (public).
3. **Observational cross-check (the only API-visible signal):** on a recent PR in each repo, confirm a `coderabbitai[bot]` review/summary actually landed:
   ...
   A non-zero count is positive evidence the author seat is working on that repo. A zero on a repo that *has* had PRs is the red flag to chase in the dashboard. (A quiet repo with no recent PRs is inconclusive, not a failure.) On the **private** repos `coderabbit.enabled` may be `false` in `.github/review-policy.yml` — those intentionally don't run CodeRabbit (the wizard's private-repo path deletes `.coderabbit.yml`, see #248 — and `scripts/ci/check_coderabbit_config` PASSes on that state), so absence there is expected, not a seat gap.
