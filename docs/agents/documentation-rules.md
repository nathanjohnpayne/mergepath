...
Scope is repo-owned prose: the root docs, `docs/**`, `rules/`, `plans/`, and `specs/`. Generated mirrors and template or propagated surfaces (see the boundary above) are out of scope. Do not reflow them; fix wrapping at their canonical source instead.
