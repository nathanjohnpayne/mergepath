...
**`enforce_admins` on the hub — declined by the owner, 2026-08-28.** The hub paragraph above argues that `mergepath` additionally needs `enforce_admins: true`, on the grounds that #427 and #428 were both admin merges past a required check. The owner declined that recommendation and set the opposite posture as explicit policy: an admin "merge without waiting for requirements to be met" escape is retained on **every** repository in the fleet, hub included. `enforce_admins` was disabled on `mergepath` `main` at 2026-08-28T02:49Z; the other eight repos already had it off, so the fleet is now uniform rather than hub-special.
