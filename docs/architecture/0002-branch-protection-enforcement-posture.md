...
The admin-enforcement portion has subsequent owner decisions: the 2026-08-28 exception below retains the hub's admin recovery path, while the [2026-09-06 consumer decision](https://github.com/nathanjohnpayne/mergepath/issues/937#issuecomment-5557027532) sets `enforce_admins: true` as the consumer target, conditional on demonstrated readiness per repository. The consumer decision supersedes the consumer half of the August exception; it does not declare that rollout has occurred.
...
**`enforce_admins` on the hub — declined by the owner, 2026-08-28.** The hub paragraph above argues that `mergepath` additionally needs `enforce_admins: true`, on the grounds that #427 and #428 were both admin merges past a required check. The owner declined that recommendation and set the opposite posture as explicit policy: an admin "merge without waiting for requirements to be met" escape was retained on **every** repository in the fleet, hub included. `enforce_admins` was disabled on `mergepath` `main` at 2026-08-28T02:49Z; the other eight repos already had it off, making the fleet uniform at that point. The hub exception remains in force; the consumer half is superseded by the staged decision below.

The reason is that the failure this ADR guards against is not the only failure available. #427/#428 are the cost of *having* the escape: a gate is bypassed that should have held. The cost of *not* having it is that a PR which has satisfied every gate on the merits cannot land when the infrastructure evaluating those gates is unavailable — and that is not hypothetical. On 2026-08-27, [#1121](https://github.com/nathanjohnpayne/mergepath/pull/1121) sat `BLOCKED` with all 36 review threads resolved, feedback accounting clear, and a cross-agent `APPROVED` in place, because three `Codex P1 unresolved threads` check-runs had failed with `rc=2` — a config/usage error, not a gate verdict — after the GitHub App installation rate limit was exhausted repo-wide by 411 workflow runs in 16 minutes. The gate's own log read `Codex blocking-tier unresolved: 0`. With `enforce_admins: true` there was no override available at all: an admin merge and a web-UI merge are refused identically, so the only remedy was to wait for an hourly quota window to roll. Two other sessions hit the same class of stall the same day.

Both failures are real; the posture is a choice about which one to accept. The owner's position is that an outage-induced deadlock across a fleet operated largely by automated agents is the worse and more frequent of the two, and that the #427/#428 class is better addressed by the gates that now exist and did not then — `Merge clearance gate` itself, the label gate, and `scripts/hooks/gh-pr-guard.sh`, which still demands an explicit `BREAK_GLASS_MERGE_STATE=1` before it will run a merge against a non-`CLEAN` PR. That hook is a local control rather than a server-side one and does not constrain a web-UI merge; it is a speed bump and is described here as one, not as a replacement for `enforce_admins`.

Consequence for the audit: `scripts/audit-branch-protection.sh --fleet` no longer passes `--require-admin-enforcement` for any repo, so an unconstrained admin is no longer reported as drift on the hub. The flag is retained for ad-hoc use. The five canonical required checks are unaffected on every repo, as are `strict`, conversation resolution, and approval counts — this exception is scoped to admin bypass alone.

### Consumer admin enforcement — accepted target, staged rollout

The owner's [DECISION v2 on #937, 2026-09-06](https://github.com/nathanjohnpayne/mergepath/issues/937#issuecomment-5557027532), and its [scope addendum](https://github.com/nathanjohnpayne/mergepath/issues/937#issuecomment-5557051117) govern consumer admin enforcement. `enforce_admins: true` is the target for consumers whose required-check set includes all five canonical checks. Repositories without branch protection are outside this rollout. Until a consumer demonstrates readiness, `enforce_admins: false` is an intentional temporary recovery path, not a permanent exemption or the normal merge path.

Before enabling enforcement, each consumer must complete a representative PR through the normal externally reviewed, gated path. A trivial documentation-only PR that bypasses that machinery does not qualify. All of these conditions must hold:

1. The final HEAD receives all five canonical required checks, and each reports success.
2. Required external-review approval or equivalent Mergepath clearance is produced through the normal automated or documented path.
3. GitHub's actual branch protection allows the merge without weakening protection, removing blocking state, manufacturing replacement checks, or bypassing the normal flow.
4. The PR merges through the intended path, with no admin bypass used at any point to pass an unsatisfied requirement.
5. No unresolved infrastructure failure remains that would have blocked that PR with admin enforcement enabled.

After a passing canary, re-confirm the repository's required-check set, enable `enforce_admins: true` for that consumer, and update the audit to require admin enforcement for it. Then complete another representative PR with the bypass unavailable. Repeat per consumer; a failure on the first enforced consumer stops the rollout until its cause is understood. This documentation does not activate enforcement or change the audit's current expectations.

A failed canary is a valid readiness result. Record the blocking condition on #937, route remediation to the existing issue owning that subsystem (or a new issue only for a distinct defect), and retain `enforce_admins: false` until a later canary passes. #937 owns readiness verification and staged enforcement, not repair of every merge-flow defect it exposes. If an admin recovery merge is needed, record the reason, link the underlying defect and obtain whatever review can safely be completed; that run cannot count as readiness evidence.

If activation exposes a defect that prevents legitimate merges, temporarily restore `enforce_admins: false`, record the incident, and repair the underlying flow before retrying. Rollback restores availability without changing the target policy. #937 is complete only when every applicable consumer has passed both the pre-enforcement canary and the post-enforcement run, and the fleet audit requires enforcement for those consumers.
