...
Most of what the refusal needs is free, because it is a property of the runs already being listed. A check run counts only if GitHub Actions produced it, its open-PR association set contains this PR alone, and it started at or after this PR was created. The association is not provenance: it names currently open PRs sharing the commit. Requiring this PR alone excludes ambiguous runs from another open PR, including runs started after both PRs existed; the creation-time floor separately excludes runs inherited from a closed PR's reused SHA. Same-named runs from another App cannot satisfy the app-pinned requirements either. These checks use the existing listing rather than a separate `commits/{sha}/pulls` query, which omits closed PRs for a commit off the default branch.
