...
| both reported and the premise holds, so it refuses | 3: the PR read, the check-run listing, the head confirmation |
| both reported but the premise is uncertain, so it nudges anyway | 4: those three plus the pre-write re-read |


It does carry one fence, and one check that looks like a fence and is not. The distinction matters because the rejected design's fences are half the reason it was rejected. Those existed because it *published verdicts*: they arbitrated ownership between two producers of one context, over a check-run set, with a compare-and-swap and a residual write window that no available primitive could close. This one is an ordinary lost-update guard—the body write replaces the whole description, so it is re-read immediately beforehand and the run aborts if it moved. Every tool that rewrites a whole PR body needs that, verdicts or not, and it is one read and one comparison rather than a three-fence protocol. It aborts rather than rebuilding, because rebuilding would re-run the validators and reopen the same window one layer down.

The one that is not a fence guards the *refusal*, not the write, and it fails in the opposite direction.

Most of what the refusal needs is free, because it is a property of the runs already being listed. A check run counts as the canonical producer having reported only if it also carries GitHub Actions as its app and an association with this PR. Both matter and both were review findings: branch protection pins these contexts to Actions, so a same-named run from another App never satisfies them; and check runs attach to a commit and outlive the PR that made them, so a closed PR's leftovers—measured on this repository as `pull_requests: []`—would otherwise hand a new PR reusing that SHA a green it was never classified for. Asking each run who it belongs to also beats asking `commits/{sha}/pulls` who else is at the head, since that endpoint omits closed PRs for a commit off the default branch and is blind to the very case it would be there for.

What is left needs one read: the head may have moved since the listing, making the whole answer describe a superseded commit. It is confirmed last, with no request after it.
