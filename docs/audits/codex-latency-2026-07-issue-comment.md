...
| `codex.reaction_freshness_window_seconds` | 1800 | 29/33 clearances swept ≤1800s — but only via event runs (p50 54s); cron-only path p50 ≈ 62m | Keep 1800s; finish the event-driven re-arm (#620) instead of widening. Covering the cron path would need ≥5400s and weakens staleness protection. |
