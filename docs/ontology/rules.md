...
This catalog documents every normative rule of Mergepath in English, in one place, with a stable ID per rule. It is a **derived model**, like the root [`CONTEXT.md`](../../CONTEXT.md) glossary: the canonical sources — `REVIEW_POLICY.md`, `AGENTS.md`, `.github/review-policy.yml`, `rules/repo_rules.md`, `ai_agent_tooling_standard.md`, the `docs/agents/` tree, and the propagation manifest — remain the normative homes, and on any divergence the source wins. Each entry cites its source; nothing here adds a rule that does not exist at a source.
