...
**R-106.** The only non-Dependabot unattended merge path is an activated #1058 native merge queue. The target has zero `merge_group` triggers. Its trusted-default-branch `pull_request_target` observer handles `auto_merge_enabled` and `enqueued`, writing only the governing author's exact-head marker and exact post-enqueue entry attestation after live checks. A separately protected organization-owned public policy repository supplies SHA-pinned authorization and repo-lint required workflows. The organization workflow ruleset activates first; the repository singleton `ALLGREEN` queue ruleset activates second, and rollback removes them in reverse order. Admission neutrally passes drafts but rejects SHA-like heads, stacks, and all target-workflow deltas. Read-only exact-target-base precheck and postcheck jobs surround one source-pinned job-scoped `contents: write` handshake that fetches public source without credentials, dispatches one nonce-bound target audit without retrying ambiguity, requires its unique author marker plus exact successful time-bound run, and then replays every public predicate and the final binding. ● — specs/merge_group_required_checks.md

**R-107.** `AUTHOR_MERGE_TOKEN`, `MERGE_QUEUE_POLICY_TOKEN`, and `MERGE_QUEUE_SOURCE_TOKEN` are pairwise distinct and exist only in the protected `merge-queue-policy` environment, never as repository or organization secrets. The environment disables administrator bypass, has no reviewers or wait timer, and permits only custom branch `main`; complete three-scope inventory reads prove placement. Candidate-controlled workflows reference none. The target-only policy PAT performs only read-only REST `GET` requests and fixed GraphQL queries with organization/target `Administration: write` plus target `Actions`, `Secrets`, `Environments`, `Contents`, `Issues`, and `Pull requests` read; the public-source-only PAT has only `Actions: read`. Exact runtime guards precede environment access in the target final auditor. The trusted observer and final auditor are the only #1058 author-token writers, limited to marker/attestation and nonce-bound final-marker comments; no token authorizes an agent to arm, enqueue, dequeue, directly merge, remove `human-hold`, disable native auto-merge, or mutate protection. ● — REVIEW_POLICY.md § Agent prohibitions
...
**R-110.** Every approval-readiness evaluation requires an existing non-author latest-state approval; a push with no approval never becomes ready, no evaluation creates a durable arm, and entry into #1058 begins with a separate human owner action. Rollout is entirely human-only: implementation and policy merges, public organization-source/environment/ruleset setup, secret placement, native arm and enqueue, and `human-hold` removal are never agent actions. The live canary proves a single nonce-bound dispatch whose auditor run has `actor.login` and `triggering_actor.login` exactly `github-actions[bot]`, exact pre-secret runtime guards, a unique marker, exact successful time-bound run, post-audit predicate replay, and final binding; it stops on retained arm state, missing evidence, exhausted polling, dispatch ambiguity without an exact response, post-audit failure, failed native ejection, unexpected merge, or automatic re-enrollment. Rollback keeps both rules active through the disabled transition, disables and drains the queue, proves no live arms, removes the repository queue rule, and only then removes the organization workflow rule. ● — REVIEW_POLICY.md § Native merge-queue boundary

**R-111.** Every readiness gate re-applies on the new HEAD and governing base. #1099 returns success with an arm intact only on the exact queue-proven required-workflow proof. From final-audit handoff through native merge, every mutable ruleset, branch-protection, environment/secret, target/public-source-workflow, rollout, PR/review/comment/thread/accounting, check/status, queue/action, default-base, and group input stays frozen; any mutation ejects or aborts the entry and requires a fresh owner action and group. GitHub has no conditional native auto-merge disable or compare-and-swap primitive, so every other non-Dependabot armed state remains untouched and returns nonzero for explicit human/admin disposition, including `rc 4`, `rc 3`, `rc 5`, missing helper or credential, stale trusted base, blocker, unreadable topology, and a newer arm; an unarmed state continues. Dependabot remains under its dedicated lane. ● — REVIEW_POLICY.md § Native merge-queue boundary
