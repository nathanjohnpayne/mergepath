...
3. **Canary first, always.** Treat the canary's propagation PR as the integration test, and don't fan out until it clears the full review ceremony; if the external reviewer is unavailable, follow the recorded CI-plus-lane exception in [propagation-ordering.md § Wave audit](../agents/propagation-ordering.md#wave-audit-one-scoped-review-per-wave-662) and record the unavailable review before proceeding. **Pick it by the dominant risk of the wave, not by size** — see [propagation-ordering.md § Canary selection](../agents/propagation-ordering.md#canary-selection-always-do-one-first-chosen-per-wave). "Pick the smallest consumer" was the right call for *this* rollout, whose dominant risk was a uniform templated payload, and it is the wrong default for a wave dominated by per-consumer configuration divergence — there the smallest repo is the least likely to surface it.
